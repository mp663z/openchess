...
A new public or read-only operation must appear in the corresponding source allowlist; the comparison ignores only that exact set of new names while comparing the rest of the contract unchanged. Order is not significant, but duplicates are refused. An existing operation cannot be moved into or out of either allowlist within a MINOR. A new area containing new operations is an allowed addition, reading the source line 14 list of new operations.
