...
A new public or read-only operation must be appended to the corresponding source allowlist; the comparison ignores only those exact appended names while comparing the rest of the contract unchanged. An existing operation cannot be moved into or out of either allowlist within a MINOR.
