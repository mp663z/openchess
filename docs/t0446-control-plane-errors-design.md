...
A later clarification makes the check truly unanchored at both edges: every start and end offset is considered, without skipping an adjacent digit or piece. Greedy regex matching alone misses a valid shorter window when the suffix adds a piece.

The final check is linear time in input length: slash segmentation and eight-segment sliding, with first/last rank suffix/prefix searches bounded by the eight-square rank sum. A stress row at the declared collection bound protects against the previously quadratic all-offset implementation.
