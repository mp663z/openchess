...
_SET_ON = {side: _C["target"]["set_on"][name]
           for side, name in (("w", "white"), ("b", "black"))}
_MOVER = {side: _C["capture"]["mover"][name]
          for side, name in (("w", "white"), ("b", "black"))}
...
def _exact_dict(value, keys):
    ...
    return type(value) is dict and \
        all(type(k) is str for k in dict.keys(value)) and \
        set(dict.keys(value)) == keys
...
def _read_state(state):
    ...
    for square, token in dict.items(occupied):
        if type(square) is not str or square not in _SQUARES or \
                type(token) is not str or token not in _PIECES:
            _fail("target_malformed")
        occ[square] = token
...
def _read_move(move):
    ...
    if type(frm) is not str or frm not in _SQUARES or \
            type(to) is not str or to not in _SQUARES:
        _fail("target_malformed")
    return kind, frm, to
    ...
    for sq, token in occ.items():
        ...
        if piece == "p":
            ...
            if max(abs(df), abs(dr)) == 1:
                return True
        elif ((piece in "rq" and (df == 0 or dr == 0)) or
              (piece in "bq" and abs(df) == abs(dr))) and (df, dr) != (0, 0) and \
                _clear(occ, square, sq, (df > 0) - (df < 0), (dr > 0) - (dr < 0)):
            return True
    return False
...
def _capture(target, occ, side, frm, to):
    ...
    if occ.get(captured) != _OTHER[side] + "p":
        _fail("target_inconsistent")
    if to != target or frm[1] != mover["mover_rank"] or \
            abs(ord(frm[0]) - ord(target[0])) != 1 or occ.get(frm) != side + "p":
...
def apply(state, move):
    ...
    set_on = _SET_ON[side]
    if kind == "pawn-advance" and token[1] == "p" and frm[0] == to[0] and \
            frm[1] == set_on["from_rank"] and to[1] == set_on["to_rank"]:
...
def turn_transition(move_type, side):
    ...
    after black's move."""
    if type(move_type) is not str or move_type not in _MOVE_TYPES or \
            type(side) is not str or side not in _OTHER:
        _fail("target_malformed")
    pawnish = move_type in ("pawn-advance", "ep-capture")
    return {
        "halfmove_clock": "reset" if pawnish and _TURN["halfmove_clock"] == "reset"
        else "increment",
        "fullmove_number": "increment" if (
            _TURN["fullmove_number"] == "increment-when-black" and side == "b")
        else "same",
    }
