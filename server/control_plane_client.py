...
OPS = _load_ops()
_AUTH_CODES = frozenset(("auth_expired", "auth_invalid"))
...
def _exact_keys(payload):
    """True when PAYLOAD is a plain dict whose keys are all plain str.
    Iterating a plain dict never calls a method of its keys."""
    return type(payload) is dict and all(type(k) is str for k in payload)


def _project(payload, fields):
    """The declared contract fields of an already-validated PAYLOAD, rebuilt
    from plain values. Undeclared keys are dropped, so no value the contract
    did not type is ever copied or returned."""
    out = {}
    for name, spec in fields.items():
        if name not in payload:
            continue
...
def _contains(payload, needles):
    """True when a plain str anywhere in PAYLOAD (plain dicts and lists,
    keys included) contains a needle, or a plain key names a write-only
    field. Iterative with a seen-set: deep or cyclic nesting cannot raise."""
...
class ControlPlaneClient:
    ...
    def _check_error(op, payload, secrets_):
        if not _exact_keys(payload):
            _fail("internal", "error response has a non-plain key")
        error = payload.get("error") if "error" in payload else None
        if not _exact_keys(error):
            _fail("internal", "error response without a plain error object")
