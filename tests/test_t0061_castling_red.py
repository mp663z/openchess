"""T0061: the castling red suite must be RED for exactly the right
reason - the T0062 runtime (tools/castling_runtime) does not exist -
while executing every T0060 fixture case, and must stay out of the
default gate's collection.

Flip hook: the moment tools/castling_runtime.py exists this harness
FAILS (stale red claim); the T0062 PR turns the red suite green and
...
RED_SUITE = ROOT / "tests" / "red" / "castling_behavior.py"
RUNTIME = ROOT / "tools" / "castling_runtime.py"
FIXTURE = json.loads((ROOT / "tests" / "fixtures" / "castling" / "cases.json").read_text())

TOTAL = sum(len(FIXTURE[s]) for s in ("happy", "boundary", "malformed", "rollback"))
DEFERRED = 0  # the castling fixture declares no deferred cases
RED = TOTAL - DEFERRED
# ... pinned to the T0060-reviewed fixture state: a fixture edit
...
RED_SUITE_SHA256 = "2130166d946d734f3cb2c2d91453cbc694f72d68ce4422f2315b84a1913b72d1"
...
def test_red_suite_exists_and_stays_out_of_default_collection():
    ...
    assert "castling_behavior" not in out.stdout, "red suite leaked into gate collection"


def test_runtime_not_implemented_yet():
    # import semantics, not one path: a package form
    # (tools/castling_runtime/__init__.py) must trip the gate just as a
    # module file does
    assert not RUNTIME.exists(), (
        "tools/castling_runtime.py exists: the T0061 red claim is stale - "
        "turn the red suite green and flip this harness in the T0062 PR"
    )
    assert importlib.util.find_spec("tools.castling_runtime") is None, (
        "tools.castling_runtime is importable: the T0061 red claim is "
        "stale - turn the red suite green and flip this harness in the T0062 PR"
    )
...
def test_continuity_digests_pin_fixture_and_red_suite_bytes():
    fx = hashlib.sha256((ROOT / "tests" / "fixtures" / "castling" / "cases.json").read_bytes())
    rs = hashlib.sha256(RED_SUITE.read_bytes())
    assert fx.hexdigest() == FIXTURE_SHA256, (
        "fixture bytes drifted from the T0060-reviewed digest - a legitimate "
        "fixture change updates FIXTURE_SHA256 in the same PR"
    )
    assert rs.hexdigest() == RED_SUITE_SHA256, (
        "red suite bytes drifted from the T0061-reviewed digest - a legitimate "
...
def test_fixture_counts_internally_consistent():
    ...
    assert (TOTAL, DEFERRED, RED) == (TOTAL_PIN, DEFERRED_PIN, RED_PIN), (
        f"fixture drifted from the T0060-reviewed counts: derived "
