...
- P-precedence: with several faults, the class follows the current order
  ...
  target, target rank, victim, capture geometry, pin.
...
def _state(rng):
    while True:
        state = _raw_state(rng)
        # the target names the square the pawn passed over, so it is empty;
        # an occupied target square is a parked finding (see T0072.md)
        if state["ep_target"] not in state["occupied"]:
            return state
...
EXPECTED_KILLS = {
    ...
    "destination-check-off": ["P-capture-sound", "P-total"],
