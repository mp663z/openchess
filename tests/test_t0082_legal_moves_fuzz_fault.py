...
class ReprRaises:
    """Any rendering (repr, str, format) runs user code: logged and raising."""
    ...
    def __repr__(self):
        return self._hit("repr")

    def __str__(self):
        return self._hit("str")

    def __format__(self, spec):
        return self._hit("format")


class StrReprRaises(str):
    """A str subclass whose rendering runs user code."""
...
class _RenderHooks:
    """Mixin: repr, str and format run user code, logged and raising."""
...
class RenderDict(_RenderHooks, dict):
    pass


class RenderList(_RenderHooks, list):
    pass


# Container render forms: the value that stands in for state, occupied or
# move itself. Each builder takes the good mapping it replaces.
CONTAINER_RENDER_FORMS = {
    "object": lambda good: ReprRaises(),
    "dict-subclass": lambda good: RenderDict(good),
    "list-subclass": lambda good: RenderList(good.items()),
}


RENDER_FORMS = {"object": lambda text: ReprRaises(), "str-subclass": StrReprRaises}
...
def _hostile_rows():
    ...
    for form, make in RENDER_FORMS.items():
        rows.append(
            (
                f"occupied-key-render-{form}",
                {**base, "occupied": {**occ, make("a4"): "wp"}},
                GOOD_MOVE,
                None,
            )
        )
        rows.append(
            (
                f"token-render-{form}",
                {**base, "occupied": {**occ, "a4": make("wp")}},
                GOOD_MOVE,
                None,
            )
        )
        rows.append((f"side-render-{form}", {**base, "side_to_move": make("w")}, GOOD_MOVE, None))
        for field in ("from_square", "to_square"):
            rows.append(
                (
                    f"move-render-{form}-{field}",
                    base,
                    {**GOOD_MOVE, field: make(GOOD_MOVE[field])},
                    "malformed_move",
                )
            )
        rows.append(
            (
                f"move-render-{form}-promotion",
                base,
                {"from_square": "b7", "to_square": "b8", "promotion": make("q")},
                "malformed_move",
            )
        )
    rows.append(
...
def _check_hostile_is_attacked(module):
    """is_attacked's square and attacker arguments, and occupied keys on
    the other entry points, with raising rendering."""
    for form, make in RENDER_FORMS.items():
        for label, square, attacker in (
            ("square", make("a1"), "w"),
            ("attacker", "a1", make("w")),
        ):
            state = copy.deepcopy(BASE)
            snap = _snap(state)
            got, calls = _render_call(
                lambda s=state, q=square, a=attacker: _typed(
                    module, lambda: module.is_attacked(s, q, a)
                )
            )
            _need(got == _want_err(None), f"is_attacked render {form} {label}: {got}")
            _need(calls == [], f"is_attacked render {form} {label}: calls {calls}")
            _need(_snap(state) == snap, "is_attacked changed the state")
        for entry in ("legal_moves", "terminal_status"):
            state = copy.deepcopy(BASE)
            state["occupied"] = {**state["occupied"], make("a4"): "wp"}
            got, calls = _render_call(
                lambda s=state, e=entry: _typed(module, lambda: getattr(module, e)(s))
            )
            _need(got == _want_err(None) and calls == [], f"{entry} render {form}: {got} {calls}")


def _container_render_rows():
    """(label, entry, args builder, expected failure class), built fresh:
    state itself, occupied itself and move itself as rendering values,
    plus a ReprRaises key and a StrReprRaises key in state and in move."""
    rows = []
    entries = {
        "apply": lambda s: (s, dict(GOOD_MOVE)),
        "legal_moves": lambda s: (s,),
        "terminal_status": lambda s: (s,),
        "is_attacked": lambda s: (s, "a1", "w"),
    }
    for entry, args in entries.items():
        for form, make in CONTAINER_RENDER_FORMS.items():
            rows.append(
                (
                    f"{entry}-state-render-{form}",
                    entry,
                    lambda args=args, make=make: args(make(copy.deepcopy(BASE))),
                    None,
                )
            )
            rows.append(
                (
                    f"{entry}-occupied-render-{form}",
                    entry,
                    lambda args=args, make=make: args(
                        {**copy.deepcopy(BASE), "occupied": make(copy.deepcopy(BASE["occupied"]))}
                    ),
                    None,
                )
            )
        for kform, key in (
            ("object", lambda: ReprRaises()),
            ("str-subclass", lambda: StrReprRaises("occupied")),
        ):
            rows.append(
                (
                    f"{entry}-state-key-render-{kform}",
                    entry,
                    lambda args=args, key=key, kf=kform: args(
                        {key(): {}, "side_to_move": "w"}
                        if kf == "str-subclass"
                        else {**copy.deepcopy(BASE), key(): 1}
                    ),
                    None,
                )
            )
    for form, make in CONTAINER_RENDER_FORMS.items():
        rows.append(
            (
                f"apply-move-render-{form}",
                "apply",
                lambda make=make: (copy.deepcopy(BASE), make(dict(GOOD_MOVE))),
                "malformed_move",
            )
        )
    for kform, key in (
        ("object", lambda: ReprRaises()),
        ("str-subclass", lambda: StrReprRaises("from_square")),
    ):
        rows.append(
            (
                f"apply-move-key-render-{kform}",
                "apply",
                lambda key=key: (copy.deepcopy(BASE), {key(): "a2", "to_square": "a3"}),
                "malformed_move",
            )
        )
    return rows
...
def _check_container_render(module):
    for label, entry, build, fc in _container_render_rows():
        args = build()
        snaps = [_snap(a) for a in args]
        got, calls = _render_call(
            lambda e=entry, a=args: _typed(module, lambda: getattr(module, e)(*a))
        )
        _need(got == _want_err(fc), f"container render {label}: {got}")
        _need(calls == [], f"container render {label}: calls {calls}")
        _need([_snap(a) for a in args] == snaps, f"container render {label}: input changed")


INJECTED_PROMOTION = "zz\nINJECTED line"


def _check_promotion_not_echoed(module):
    """An out-of-enum promotion is exact str, so it cannot raise, but its
    caller-chosen content must still never be echoed into the message."""
    move = {**GOOD_MOVE, "promotion": INJECTED_PROMOTION}
    snap = _snap(move)
    got = _typed(module, lambda: module.apply(copy.deepcopy(BASE), move))
    _need(got == _want_err("promotion_forbidden"), f"promotion echo: {got}")
    try:
        module.apply(copy.deepcopy(BASE), move)
    except module.LegalMovesError as exc:
        text = " ".join([str(exc), *map(str, exc.args)])
        _need("INJECTED" not in text and "zz" not in text, "promotion echoed into the message")
    _need(_snap(move) == snap, "promotion echo: input changed")
...
def _probe(module, sweep=True):
    ...
    for label, check in (
        ...
        ("hostile-is-attacked", _check_hostile_is_attacked),
        ("container-render", _check_container_render),
        ("promotion-not-echoed", _check_promotion_not_echoed),
    ):
        try:
            check(module)
        except BaseException as exc:  # noqa: BLE001 - any escape is a failure
...
MUTANTS = {
    "promotion-repr-in-message": [
        (
            '_fail("promotion_forbidden", "promotion is outside the enum")',
            '_fail("promotion_forbidden", f"promotion {move[\'promotion\']!r}"'
            ' " is outside the enum")',
        )
    ],
    "state-type-repr-in-message": [
        (
            'if type(state) is not dict:\n        _malformed(f"state must be exactly',
            'if type(state) is not dict:\n        _malformed(f"state {state!r} must be exactly',
        )
    ],
    "state-keys-repr-in-message": [
        (
            'set(state) != _STATE_KEYS:\n        _malformed(f"state must be exactly',
            'set(state) != _STATE_KEYS:\n        _malformed(f"state {state!r} must be exactly',
        )
    ],
    "occupied-type-repr-in-message": [
        (
            '_malformed("occupied must be a mapping")',
            '_malformed(f"occupied {occ!r} must be a mapping")',
        )
    ],
    "move-type-repr-in-message": [
        (
            '_fail("malformed_move", "move must be a mapping")',
            '_fail("malformed_move", f"move {move!r} must be a mapping")',
        )
    ],
    "move-keytype-repr-in-message": [
        (
            '"move member names must be exact strings"',
            'f"move member names {list(move)!r} must be exact strings"',
        )
    ],
    "occupied-key-repr-in-message": [
        (
            '_malformed("occupied key is not a contract square")',
            '_malformed(f"occupied key {sq!r} is not a contract square")',
        )
    ],
    "token-repr-in-message": [
        ('_malformed("bad piece token")', '_malformed(f"bad piece token {tok!r}")')
    ],
    "square-repr-in-message": [
        (
            '_malformed("square is not a contract square")',
            '_malformed(f"square {square!r} is not a contract square")',
        )
    ],
    "side-repr-in-message": [
        (
            '_malformed(f"side_to_move must be one of {list(_SIDES)}")',
            '_malformed(f"side_to_move {stm!r} must be one of {list(_SIDES)}")',
        )
    ],
    "move-square-repr-in-message": [
        (
            '_fail("malformed_move", "squares must match the square grammar")',
            '_fail("malformed_move", f"squares {fr!r} {to!r} must match the square grammar")',
        )
    ],
...
CONSTRUCTOR_MUTANTS = {
    "constructor-code-repr-in-message": [
        (
...
def test_mutants_apply_exactly_once():
    for name, edits in {**MUTANTS, **CONSTRUCTOR_MUTANTS}.items():
...
def _constructor_render_ok(module):
    """Out-of-shape values whose rendering raises still give ValueError,
    with no rendering call."""
    for field in ("code", "failure_class", "message"):
        for make in RENDER_FORMS.values():
            kwargs = {"code": "illegal_move", "message": "m", "failure_class": "no_piece"}
            kwargs[field] = make("nope")

            def build(k=kwargs):
                try:
                    module.LegalMovesError(**k)
                except ValueError:
                    return "ValueError"
                return "accepted"

            got, calls = _render_call(build)
            _need(got == "ValueError" and calls == [], f"constructor {field}: {got} {calls}")


def test_error_constructor_never_renders_its_arguments():
    _constructor_render_ok(rt)


def test_constructor_repr_mutant_is_red():
    name = "constructor-code-repr-in-message"
    with pytest.raises(Fail):
        _constructor_render_ok(_source_mutant(name, CONSTRUCTOR_MUTANTS[name]))
...
def test_hostile_rendering_rows_on_every_entry_point():
    _check_hostile_is_attacked(rt)


def test_container_render_rows():
    _check_container_render(rt)


def test_promotion_is_never_echoed():
    _check_promotion_not_echoed(rt)
