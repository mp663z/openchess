...
# Hostile str forms (coordinator standing rule): a plain subclass, one whose
# __eq__ raises and one whose __hash__ collides with a given text. Every str
# method they could run is logged, so a row can pin that no caller code ran.
HOSTILE_CALLS = []
_LOGGED = (
    "__eq__",
    "__ne__",
...
def hostile_str(form, text, collide_with="x"):
    ...
    cls = type(f"Hostile_{form.replace('-', '_')}", (str,), {n: logged(n) for n in _LOGGED})
    return cls(text)
...
@pytest.mark.parametrize("form", HOSTILE_FORMS)
def test_r4_hostile_str_payload_forms(form):
    payload = hostile_str(form, START, collide_with="w")
    kind = type(payload)
    HOSTILE_CALLS.clear()
    assert _typed_malformed(prod, payload)
    assert HOSTILE_CALLS == []
    assert type(payload) is kind and str.__eq__(payload, START)
...
EQUIVALENT_EDITS = {
    # the typed error is still fresh, not forged and has no __cause__;
    # __context__ is not asserted (wal ruling)
    "int-limit-fails-inside-handler": (
        "            number = None\n",
        '            fail("malformed_fen")\n',
    ),
