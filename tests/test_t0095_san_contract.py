...
def _state(board: str, side: str) -> dict:
...
def _castling_moves(state):
    """Castling candidates per the castling contract's denotation:
    king two squares toward the rook with the rook hop. Legality:
    king + rook on start squares, empty path, king's transit squares
    unattacked. (Rights availability is a caller precondition here;
    the castling contract owns the full model.)"""
    ...
    if _king_square(occ, side) in attacked:
        return out
    for dest, rook_sq, between, transit in (
        ("g" + rank, "h" + rank, ["f" + rank, "g" + rank],
         ["f" + rank, "g" + rank]),
        ("c" + rank, "a" + rank, ["b" + rank, "c" + rank, "d" + rank],
         ["d" + rank, "c" + rank]),
    ):
        if occ.get(rook_sq) != r:
            continue
        if any(sq in occ for sq in between):
            continue
        if any(sq in attacked for sq in transit):
            continue
        out.append({"from_square": "e" + rank, "to_square": dest})
    return out
...
def resolve_san(contract, text, state):
    ...
    if body in (t["castling_kingside"], t["castling_queenside"]):
        ...
        cand = [mv for mv in moves if matches(mv)]
    ...
    # suffix pinned to the RESULTING position
    new_state = {"occupied": _apply(occ, move),
                 "side_to_move": enemy}
...
def emit_san(contract, move, state):
    ...
    new_state = {"occupied": _apply(occ, move), "side_to_move": enemy}
    terminal = _terminal_status(new_state)
...
@pytest.mark.parametrize("san,board,side", [
    # no knight can reach e5 from startpos
    ("Ne5", STARTPOS_BOARD, "w"),
    # pawn capture with nothing to capture
    ("exd5", STARTPOS_BOARD, "w"),
    # false check claim
    ("e4+", STARTPOS_BOARD, "w"),
    # false mate claim
    ("e4#", STARTPOS_BOARD, "w"),
...
def test_no_legal_match_rejected(san, board, side):
    doc = _doc()["contract"]
    with pytest.raises(SanError) as exc:
        resolve_san(doc, san, _state(board, side))
