...
_THREADS = "option name Threads type spin default 1 min 1 max 512"
_ID = [("gui", "uci"), ("engine", "id name S")]
_READY_OPT = [*_ID, ("engine", _THREADS), ("engine", "uciok")]
...
LOCAL_SESSION = {
    ...
    # handshake order (Session._handshake)
    "option-before-id-name": ([("gui", "uci")], "engine", _THREADS),
    "uciok-before-id-name": ([("gui", "uci")], "engine", "uciok"),
    "id-name-twice": (_ID, "engine", "id name T"),
    "id-author-after-option": ([*_ID, ("engine", _THREADS)], "engine", "id author A"),
    "id-after-uciok": (_READY_OPT, "engine", "id name T"),
    "option-after-uciok": (
        _READY_OPT,
        "engine",
        "option name Hash type spin default 1 min 1 max 64",
    ),
    # setoption phase (lifecycle table)
    "setoption-before-uciok": (_ID, "gui", "setoption name Threads value 4"),
    # registration phase (Session._registration)
    "registration-before-uciok": (_ID, "engine", "registration checking"),
    "registration-ok-unsolicited": (_READY_OPT, "engine", "registration ok"),
    "registration-checking-twice": (
        [*_READY_OPT, ("engine", "registration checking")],
        "engine",
        "registration checking",
    ),
    # GUI register (Session._register)
    "register-before-uciok": (_ID, "gui", "register later"),
    "register-before-indication": (_READY_OPT, "gui", "register name X code Y"),
    "register-later-before-indication": (_READY_OPT, "gui", "register later"),
    "register-after-registered": (
        [*_READY_OPT, ("engine", "registration checking"), ("engine", "registration ok")],
        "gui",
        "register name X code Y",
    ),
    # debug phase (Session._debug)
    "debug-before-uci": ([], "gui", "debug on"),
    "debug-before-uciok": (_ID, "gui", "debug on"),
    "debug-after-quit": ([*_READY_OPT, ("gui", "quit")], "gui", "debug on"),
}

# registry rejections inside a live session: malformed_line, the session
# unchanged (Session._declare_option, Session._setoption)
LOCAL_SESSION_MALFORMED = {
    "option-duplicate": ([*_ID, ("engine", _THREADS)], "engine", _THREADS),
    "option-case-collision": (
        [*_ID, ("engine", _THREADS)],
        "engine",
        "option name threads type check default false",
    ),
    "setoption-undeclared": (_READY_OPT, "gui", "setoption name Hash value 4"),
    "setoption-spin-out-of-range": (_READY_OPT, "gui", "setoption name Threads value 513"),
    "setoption-spin-no-value": (_READY_OPT, "gui", "setoption name Threads value"),
}


# contract rules the reference's own lists leave unpinned: promotion_letters
# is exactly [q, r, b, n] (no uppercase); the token separator is a single
# space (a tab is not one); squares are lowercase file + rank; (none) is only
# the empty bestmove slot, never a move token elsewhere
EXTRA_MALFORMED_GUI = [
    "position startpos moves e2e4 e7e8Q",
    "go\tinfinite",
    "isready\t",
    "position startpos moves E2E4",
    "position startpos moves (none)",
]
EXTRA_MALFORMED_ENGINE = [
    "bestmove a7a8Q",
    "info\tdepth 1",
    "bestmove E2E4",
    "info pv (none)",
...
# other whitespace inside a line is not a separator, so the first token is
# an unknown keyword (the reference's class)
EXTRA_UNKNOWN = ["go\x0binfinite", "go\xa0infinite"]
...
def _local_rows():
    ...
    for line in uci.MALFORMED_GUI + EXTRA_MALFORMED_GUI:
    ...
    for line in uci.UNKNOWN + EXTRA_UNKNOWN:
        rows["local-unknown:" + line] = ("parse_fail", "gui", line, _want("unknown_command"))
    for line in uci.MALFORMED_ENGINE + EXTRA_MALFORMED_ENGINE:
    ...
    for name, (steps, direction, line) in LOCAL_SESSION_MALFORMED.items():
        rows["local-session:" + name] = ("session", steps, direction, line, _want("malformed_line"))
    return rows
...
def _promotion_letter_case_folded(contract, token):
    if type(token) is str and len(token) == 5:
        token = token[:4] + token[4].lower()
    return _is_move(contract, token)


def _tab_as_separator(contract, line):
    return _check_line(contract, line.replace("\t", " ") if type(line) is str else line)


def _square_case_folded(contract, token):
    if type(token) is str and len(token) in (4, 5):
        token = token[:4].lower() + token[4:]
    return _is_move(contract, token)


def _none_token_as_move(contract, token):
    return token == "(none)" or _is_move(contract, token)


def _other_whitespace_as_separator(contract, line):
    if type(line) is str:
        line = re.sub("[\x0b\x0c\xa0]", " ", line)
    return _check_line(contract, line)
...
MUTANTS = {
    ...
    "promotion-letter-case-folded": {(M, "_is_move"): _promotion_letter_case_folded},
    "tab-as-separator": {(M, "_check_line"): _tab_as_separator},
    "square-case-folded": {(M, "_is_move"): _square_case_folded},
    "none-token-as-move": {(M, "_is_move"): _none_token_as_move},
    "other-whitespace-as-separator": {(M, "_check_line"): _other_whitespace_as_separator},
    ...
    "debug-rejection-swallowed": {(S, "_debug"): _rejection_swallowed(S._debug)},
    "declare-option-rejection-swallowed": {
        (S, "_declare_option"): _rejection_swallowed(S._declare_option)
    },
    "setoption-rejection-swallowed": {(S, "_setoption"): _rejection_swallowed(S._setoption)},
    "handshake-rejection-swallowed": {(S, "_handshake"): _rejection_swallowed(S._handshake)},
    "registration-rejection-swallowed": {
        (S, "_registration"): _rejection_swallowed(S._registration)
    },
    "register-rejection-swallowed": {(S, "_register"): _rejection_swallowed(S._register)},
...
EXPECTED_KILLS = {
    ...
    "edges-stripped": [
        "local-malformed-gui: uci",
        "local-malformed-gui:isready\t",
        "local-malformed-gui:uci ",
    ],
    ...
    "promotion-letter-case-folded": [
        "local-malformed-engine:bestmove a7a8Q",
        "local-malformed-gui:position startpos moves e2e4 e7e8Q",
    ],
    "tab-as-separator": [
        "local-malformed-engine:info\tdepth 1",
        "local-malformed-gui:go\tinfinite",
    ],
    "square-case-folded": [
        "local-malformed-engine:bestmove E2E4",
        "local-malformed-gui:position startpos moves E2E4",
    ],
    "none-token-as-move": [
        "local-malformed-engine:bestmove e2e4 ponder (none)",
        "local-malformed-engine:info pv (none)",
        "local-malformed-gui:position startpos moves (none)",
    ],
    "other-whitespace-as-separator": [
        "local-unknown:go\x0binfinite",
        "local-unknown:go\xa0infinite",
    ],
    ...
    "unlisted-pair-ignored": [
        ...
        "local-session:id-after-uciok",
        "local-session:option-after-uciok",
        "local-session:ponderhit-while-searching",
        "local-session:setoption-before-uciok",
    ...
    "debug-rejection-swallowed": [
        "local-session:debug-after-quit",
        "local-session:debug-before-uci",
        "local-session:debug-before-uciok",
    ],
    "declare-option-rejection-swallowed": [
        "local-session:option-case-collision",
        "local-session:option-duplicate",
    ],
    "setoption-rejection-swallowed": [
        "local-session:setoption-spin-no-value",
        "local-session:setoption-spin-out-of-range",
        "local-session:setoption-undeclared",
    ],
    "handshake-rejection-swallowed": [
        "local-session:id-author-after-option",
        "local-session:id-name-twice",
        "local-session:option-before-id-name",
        "local-session:uciok-before-id-name",
    ],
    "registration-rejection-swallowed": [
        "local-session:registration-before-uciok",
        "local-session:registration-checking-twice",
        "local-session:registration-ok-unsolicited",
    ],
    "register-rejection-swallowed": [
        "local-session:register-after-registered",
        "local-session:register-before-indication",
        "local-session:register-before-uciok",
        "local-session:register-later-before-indication",
    ],
    "reject-mutates-session": [
        "duplicate-uci",
