...
# the landed capturing pawn is the ONLY blocker between the king and a
# slider: the capture is legal only because the pawn stands on the target
# square before the king-safety check (file and diagonal, both colours)
EP_BLOCKER_ROWS = [
    "4r2k/8/8/3Pp3/8/8/8/4K3 w - e6 0 1",  # e-file: Re8 / Ke1, dxe6 blocks
    "6bk/8/8/3Pp3/8/8/K7/8 w - e6 0 1",  # diagonal g8-a2 through d5 and e6
    "3k4/8/8/8/3Pp3/8/8/3R2K1 b - d3 0 1",  # d-file: Rd1 / Kd8, exd3 blocks
    "8/7k/8/8/3Pp3/8/8/1B2K3 b - d3 0 1",  # diagonal b1-h7 through d3 and e4
]

# the only capturer stands on the a-file (the lower edge of the neighbour
# file bounds), both colours: the target must be kept
EP_A_FILE_ROWS = [
    "4k3/8/8/Pp6/8/8/8/4K3 w - b6 0 2",
    "4k3/8/8/8/pP6/8/8/4K3 b - b3 0 2",
]

# ... and on the h-file (the upper edge), both colours
EP_H_FILE_ROWS = [
    "4k3/8/8/6pP/8/8/8/4K3 w - g6 0 2",
    "4k3/8/8/8/6Pp/8/8/4K3 b - g3 0 2",
]

# two capturers: the first one probed (lower file) is pinned, the second is
# legal - the target is kept (every neighbour is probed, not only the first)
EP_SECOND_CAPTURER_ROWS = [
    "4k3/b7/8/2PpP3/8/4K3/8/8 w - d6 0 2",  # c5 pinned on a7-e3; exd6 legal
    "8/8/5k2/8/3pPp2/8/8/B3K3 b - e3 0 2",  # d4 pinned on a1-f6; fxe3 legal
]
...
def _ep_blocker_red(mod):
    for fen in EP_BLOCKER_ROWS + EP_A_FILE_ROWS + EP_H_FILE_ROWS + EP_SECOND_CAPTURER_ROWS:
        a = _call(mod.digest_fen, "standard", fen)
        if a != _call(ref.digest_fen, "standard", fen):
            return True
        if a == _call(mod.digest_fen, "standard", _cleared(fen)):
            return True
    return False


def test_r1_production_lint_helper_passes():
    prod._lint()


def test_r2_pinned_first_capturer_does_not_hide_the_second():
    for fen in EP_SECOND_CAPTURER_ROWS:
        a = prod.digest_fen("standard", fen)
        assert a == ref.digest_fen("standard", fen), fen
        assert a != prod.digest_fen("standard", _cleared(fen)), fen


def test_r2_edge_file_capturer_keeps_the_target():
    for fen in EP_A_FILE_ROWS + EP_H_FILE_ROWS:
        a = prod.digest_fen("standard", fen)
        assert a == ref.digest_fen("standard", fen), fen
        assert a != prod.digest_fen("standard", _cleared(fen)), fen


def test_r2_landed_pawn_as_only_blocker_keeps_the_target():
    assert not _ep_blocker_red(prod)
    for fen in EP_BLOCKER_ROWS:
        assert ref.digest_fen("standard", fen) != ref.digest_fen("standard", _cleared(fen)), fen


def _variant_encoding_red(mod):
    """encode() with a synthetic two-entry registry: the variant id is a
    canonical component, so a second id gives a different encoding and
    appears verbatim in the variant component."""
    dc, vc, ec, fc = prod._docs()
    vc2 = copy.deepcopy(vc)
    entry = copy.deepcopy(vc2["variants"]["entries"][0])
    entry["id"] = "test-variant"
    vc2["variants"]["entries"].append(entry)
    position = prod.parse_fen(fc, GOOD_FEN)
    order = vc2["identity"]["canonical_fields"]
    out = {}
    for vid in ("standard", "test-variant"):
        enc = mod.encode(dc, vc2, ec, fc, vid, position)
        if enc.split(" ")[order.index("variant")] != vid:
            return True
        out[vid] = enc
    return out["standard"] == out["test-variant"]


def test_r2_variant_is_a_canonical_component_of_the_encoding():
    assert not _variant_encoding_red(prod)


MUTANTS = {
    "ep-landed-pawn-not-placed": (
        "        after[(tf, target_rank)] = own_pawn\n",
        "",
    ),
    "ep-file-lower-bound-exclusive": (
        "        if not 0 <= f < len(files):\n",
        "        if not 0 < f < len(files):\n",
    ),
    "ep-file-upper-bound-short": (
        "        if not 0 <= f < len(files):\n",
        "        if not 0 <= f < len(files) - 1:\n",
    ),
    "ep-first-neighbour-decides": (
        "            return ep\n    return sentinel",
        "            return ep\n        return sentinel\n    return sentinel",
    ),
    "lint-helper-inverted": (
        "    assert main([str(CONTRACT)]) == 0",
        "    assert main([str(CONTRACT)]) != 0",
    ),
    "encode-variant-constant": (
        '        "variant": variant_id,',
        '        "variant": "standard",',
    ),
...
EQUIVALENT_EDITS = {
    ...
    # the neighbours are probed in either order: the target is kept if any
    # capture is legal, so the order never changes the result
    "ep-neighbour-order-swapped": ("    for df in (-1, 1):", "    for df in (1, -1):"),
    # same reason: file -1 or len(files) is never a board key either
    "ep-file-lower-bound-widened": (
        "        if not 0 <= f < len(files):\n",
        "        if not -1 <= f < len(files):\n",
    ),
    "ep-file-upper-bound-widened": (
        "        if not 0 <= f < len(files):\n",
        "        if not 0 <= f <= len(files):\n",
    ),
}
    ...
    try:
        ...
        for fen in EP_ROWS + FENS[::25]:
            if _call(mod.digest_fen, "standard", fen) != _call(ref.digest_fen, "standard", fen):
                return True
        if _ep_blocker_red(mod) or _variant_encoding_red(mod):
            return True
        mod._lint()  # the module's own contract-lint helper must pass
