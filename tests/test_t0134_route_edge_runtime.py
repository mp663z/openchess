...
# -- verifier-2 gaps: to-square grammar, bucket key, hostile str values ------

BAD_TO_SQUARE_MOVES = ("e2z4", "e2e9")


def _bad_to_square_red(mod):
    """A bad TO square is malformed_edge_record on insert, validate and
    merge (route_edge.py move grammar reads both squares)."""
    for move in BAD_TO_SQUARE_MOVES:
        rec = dict(_good_record(), move=move)
        calls = (
            lambda m=move: mod.EdgeTable(DOCS).insert("standard", m, STARTPOS, AFTER_E4),
            lambda r=rec: mod.validate_record(*DOCS, dict(r)),
            lambda r=rec: mod.EdgeTable(DOCS).merge(_Src([dict(r)])),
        )
        for call in calls:
            try:
                call()
...
def _clock_hash_oracle(variant, fen_text):
    """Deterministic oracle hashing the FULL fen text, clocks included."""
    return "pdv1:" + hashlib.sha256(f"{variant}|{fen_text}".encode()).hexdigest()


RECLOCKED_START = STARTPOS.replace(" 0 1", " 3 7")
OTHER_TARGET = AFTER_E4.replace("KQkq", "Kkq")


def _bucket_key_red(mod):
    """Under a clock-sensitive oracle the bucket key must come from the
    canonical from-snapshot: one record across clockings, and a
    different target from the second clocking is conflicting_edge."""
    t = mod.EdgeTable(DOCS, _clock_hash_oracle)
    t.insert("standard", "e2e4", STARTPOS, AFTER_E4)
    t.insert("standard", "e2e4", RECLOCKED_START, AFTER_E4)
    if len(t.records()) != 1:
        return True
    before = t.serialize()
    try:
        t.insert("standard", "e2e4", RECLOCKED_START, OTHER_TARGET)
        return True
    except mod.EdgeError as err:
        if err.failure_class != "conflicting_edge":
            return True
    return t.serialize() != before


class _HV(str):
    """str-subclass record value whose compare/hash must never run."""

    calls = []

    def __eq__(self, other):
        _HV.calls.append("eq")
        raise RuntimeError("hostile value compare")
    ...
    def __hash__(self):
        _HV.calls.append("hash")
        raise RuntimeError("hostile value hash")


def _hostile_value_red(mod):
    good = _good_record()
    for field in ("variant", "move", "from_snapshot_fen", "to_snapshot_fen"):
        rec = dict(good)
        rec[field] = _HV(good[field])
        dst = mod.EdgeTable(DOCS)
        dst.insert("standard", "d2d4", STARTPOS, STARTPOS.replace(
            "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w",
            "rnbqkbnr/pppppppp/8/8/3P4/8/PPP1PPPP/RNBQKBNR b"))
        before = dst.serialize()
        _HV.calls.clear()
        for call in (
            lambda r=rec: mod.validate_record(*DOCS, r),
            lambda r=rec, d=dst: d.merge(_Src([r])),
        ):
            try:
                call()
                return True
                ...
                if err.failure_class != MER:
                    return True
        if _HV.calls or dst.serialize() != before:
            return True
    return False


def test_r2_bad_to_square_rejects_on_insert_validate_merge():
    assert not _bad_to_square_red(prod)


def test_r2_insert_bucket_key_uses_canonical_snapshot():
    assert not _bucket_key_red(prod)


def test_r2_hostile_str_subclass_values_fail_closed_without_calls():
...
FIELDS4 = ("variant", "move", "from_snapshot_fen", "to_snapshot_fen")


def _renamed_key_red(mod):
    """Same-arity record with one renamed key per field: merge rejects
    malformed_edge_record, zero oracle calls, table unchanged."""
    for field in FIELDS4:
        rec = dict(_good_record())
        rec[field + "_x"] = rec.pop(field)
        oracle = _CountingOracle()
        dst = mod.EdgeTable(DOCS, oracle)
        before, calls = dst.serialize(), oracle.calls
        try:
            dst.merge(_Src([rec]))
            return True
        except mod.EdgeError as err:
            if err.failure_class != MER:
                return True
        if oracle.calls != calls or dst.serialize() != before:
            return True
    return False


INSERT_PRECEDENCE = [
    (("standard", 1, 1, AFTER_E4), MER),  # move type before FEN types
    (("standard", None, None, None), MER),
    (("standard", "e2e4", 1, AFTER_E4), "malformed_position"),
    (("nope", 1, 1, 1), "unknown_variant"),  # variant first
]


def _insert_precedence_red(mod):
    for args, cls in INSERT_PRECEDENCE:
        try:
...
def _validate_precedence_red(mod):
    """Unknown variant AND bad move: unknown_variant on validate and merge
    (reference order: types, variant, move)."""
    for move in ("e2e2", "e2z4", "zzzz"):
        rec = dict(_good_record(), variant="nope", move=move)
        for call in (
            lambda r=rec: mod.validate_record(*DOCS, dict(r)),
            lambda r=rec: mod.EdgeTable(DOCS).merge(_Src([dict(r)])),
        ):
...
def _non_str_values_red(mod):
    """Non-str values (int, None) in each of the 4 fields: validate and
    merge reject malformed_edge_record with zero oracle calls."""
    for field in FIELDS4:
        for value in (1, None):
            rec = dict(_good_record())
            rec[field] = value
            ...
            before = dst.serialize()
            for call in (
                lambda r=rec, o=oracle: mod.validate_record(*DOCS, dict(r), o),
                lambda r=rec, d=dst: d.merge(_Src([dict(r)])),
            ):
                try:
                    call()
                    return True
                    ...
                    if err.failure_class != MER:
                        return True
            if oracle.calls or dst.serialize() != before:
                return True
    return False


def _live_codes_red(mod):
    """.code comes from the table's own (live) ec mapping, not the module's
    frozen FAILURE_MAPPING."""
    docs = copy.deepcopy(DOCS)
    docs[0]["failures"]["mapping"]["malformed_edge_record"] = "live_code_probe"
    bad = dict(_good_record(), move="e2e2")
    for call in (
        lambda: mod.EdgeTable(docs).insert("standard", "e2e2", STARTPOS, AFTER_E4),
        lambda: mod.validate_record(*docs, dict(bad)),
        lambda: mod.EdgeTable(docs).merge(_Src([dict(bad)])),
    ):
        try:
            call()
            return True
        except mod.EdgeError as err:
            if (err.failure_class, err.code) != (MER, "live_code_probe"):
                return True
    return False


def test_r2_merge_renamed_key_fails_closed_without_oracle_calls():
    assert not _renamed_key_red(prod)


def test_r2_insert_precedence_matches_reference_order():
    assert not _insert_precedence_red(prod)
    for args, cls in INSERT_PRECEDENCE:
        assert _outcome(lambda a=args: ref.EdgeTable(DOCS).insert(*a))[1] == cls


def test_r2_validate_and_merge_variant_precedes_move():
    assert not _validate_precedence_red(prod)


def test_r2_non_str_values_all_fields_fail_closed_without_oracle_calls():
    assert not _non_str_values_red(prod)


def test_r2_failure_codes_come_from_live_docs():
    assert not _live_codes_red(prod)
...
MUTANTS = {
    ...
    "merge-validate-off": (
        "            validate_record(*self._docs(), rec, self.digest_fn)\n", ""),
    "move-checks-from-square-only": (
        "    for square in squares:\n",
        "    for square in squares[:1]:\n"),
    "bucket-key-from-raw-fen": (
        "                              variant_id, rec[\"from_snapshot_fen\"]), move)\n",
        "                              variant_id, from_fen), move)\n"),
    "record-value-isinstance": (
        "    if not all(type(record[f]) is str for f in _FIELDS):\n",
        "    if not all(isinstance(record[f], str) for f in _FIELDS):\n"),
    # formerly an equivalent edit; the verifier-1 insert-precedence rows
    # (move type before FEN types) now separate it
    "insert-move-type-off": (
        "    if type(move) is not str:\n"
        "        _fail(ec, \"malformed_edge_record\")\n", ""),
    "merge-key-set-length-only": (
        "            if not _exact_dict(rec) or set(dict.keys(rec)) != set(\n"
        "                    self.ec[\"record\"][\"fields\"]):\n",
        "            if not _exact_dict(rec) or len(dict.keys(rec)) != len(\n"
        "                    self.ec[\"record\"][\"fields\"]):\n"),
    "insert-position-type-before-move-type": (
        "    if type(move) is not str:\n"
        "        _fail(ec, \"malformed_edge_record\")\n"
    ...
    "validate-move-before-variant": (
        ...
        "    if record[\"variant\"] not in [e[\"id\"] for e in vc[\"variants\"][\"entries\"]]:\n"
        "        _fail(ec, \"unknown_variant\")\n"),
    "validate-types-three-fields": (
        "    if not all(type(record[f]) is str for f in _FIELDS):\n",
        "    if not all(type(record[f]) is str for f in _FIELDS[:3]):\n"),
    "failure-code-from-frozen-mapping": (
        "    raise EdgeError(cls, ec[\"failures\"][\"mapping\"][cls])\n",
        "    raise EdgeError(cls, FAILURE_MAPPING[cls])\n"),
    "shared-module-docs": [
