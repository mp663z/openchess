...
def test_r7_reinsert_returns_a_detached_record():
    t = oc.ContextTable()
    t.insert("standard", ["e2e4", "c7c5"])
    before = t.serialize()
    again = t.insert("standard", ["e2e4", "c7c5"])
    again["opening_name"] = "POISON"
    again["opening_code"] = "X99"
    again["path_moves"].append("g1f3")
    again["path_moves"][0] = "a2a3"
    assert t.serialize() == before
    assert t.insert("standard", ["e2e4", "c7c5"])["opening_name"] == "Sicilian Defense"


class _SubTable(oc.ContextTable):
    pass
...
def test_r5_merge_rejects_non_exact_table_sources_carrying_records():
    real = _table([["e2e4", "c7c5"]])
    sub = _SubTable()
    sub.insert("standard", ["d2d4"])
    ns = types.SimpleNamespace(_records=copy.deepcopy(real._records))
    dst = _table([["d2d4", "d7d5", "c2c4"]])
    before = dst.serialize()
    for bad in (sub, _Duck(), ns):
        assert _err(lambda bad=bad: dst.merge(bad)) == (MCR, oc.FAILURE_MAPPING[MCR])
        assert dst.serialize() == before
