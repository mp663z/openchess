...
def _oracle(name, holder):
    ...
    oracles that close over the receiver."""
    if name == "constant":
...
def _table_of(oracle_name, initial, cls=CollisionProbe):
    holder = []
    table = cls(_oracle(oracle_name, holder))
    holder.append(table)
    ...
    try:
        ...
        if section == "malformed":
            ...
            try:
                ...
                return error.failure_class != row["expect_failure"]
            return True
        table = _table_of(row["oracle"], row["initial"], cls)
        _apply(table, row["op"])
        return _pinned(table) != row["expect"]
...
FIXTURE_SHA256 = "d45c51f360b344d2a5638a73b2f4a9014254708a4bbc63119e3a2234962e0e19"
