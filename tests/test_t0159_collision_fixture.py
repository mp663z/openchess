...
OP_KINDS = {"insert": {"kind", "variant", "fen"}, "merge": {"kind", "records"},
...
ORACLES = ("constant", "real", "raw", "stateful", "raising", "interrupt", "invalid-format",
           "str-subclass", "non-str", "reentrant", "corrupt-then-raise-on-ep",
           "corrupt-then-return")
...
def _pinned(table):
    ...
    return {"records": [dict(record) for _, record in ordered],
...
def _state(table):
    ...
    return (copy.deepcopy(table.identity_index), copy.deepcopy(table.buckets),
    ...
    for section in SECTIONS:
        ...
        for row in rows:
            ...
            if section in ("happy", "boundary"):
                ...
                assert set(row) == {"name", "defect", "oracle", "initial", "op",
                                    "expect_failure", "oracle_calls", "minimal_repair"}
                ...
                assert set(row) == {"name", "why", "oracle", "initial", "bad",
                                    "expect_failure", "good", "expect"}
...
], ids=["drop-notes", "wrong-contract", "duplicate-name", "two-locus-repair",
        "undeclared-class", "unknown-oracle", "extra-op-field", "empty-rollback-table"])
...
def test_failure_class_coverage():
    ...
    assert {row["expect_failure"] for row in _rows("rollback")} >= {
        "malformed_collision_record", "accelerator_inconsistent"}
    assert {row["oracle"] for s in SECTIONS for row in _rows(s)} | {
        row["minimal_repair"].get("oracle") for row in _rows("malformed")} >= set(ORACLES)


# -- happy and boundary -----------------------------------------------------------

@pytest.mark.parametrize("section,name", [(s, n) for s in ("happy", "boundary")
                                          for n in _names(s)] if CASES else [])
    ...
    try:
        ...
        if section == "malformed":
            ...
            try:
                ...
                return error.failure_class != row["expect_failure"] or \
                    table.fixture_state["calls"] != row["oracle_calls"]
            return True
        table = _table_of(row["oracle"], row["initial"], cls)
        _apply(table, row["op"])
        return _pinned(table) != row["expect"]
...
SOURCE_MUTANTS = {
    ...
    "no-reentrancy-guard": (
        "        if self._active:\n", "        if False:\n"),
    "shape-len-not-key-set": (
        "set(rec) != _RECORD_FIELDS", "len(rec) != len(_RECORD_FIELDS)"),
    "no-semantic-phase": (
...
@pytest.mark.parametrize("name,section,row", [
...
@pytest.mark.parametrize("cls,section,name", [
...
], ids=["bucket-dedup-four", "bucket-dedup-three", "arrival-order", "no-restore",
        "trust-key-raw", "trust-key-stateful", "trust-key-rollback"])
...
FIXTURE_SHA256 = "975b0bd7f5a0586cee04abd8ce763700dd6691670fb5c63d13c1d0657fdc86a2"
