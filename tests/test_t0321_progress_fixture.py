...
def _m(name, bad, fix):  # noqa: C901 - one closed branch per tag
    ...
    if name == "request_not_a_dict":
        ...
        want = "w-2" if field == "worker" else "job1:" + "fedcba9876543210" * 4
        assert type(bad[field]) is str and bad[field] == want
        assert (_WORKER_RE if field == "worker" else _JOB_RE).fullmatch(bad[field])
        _only(bad, fix, (field,))
...
_CLOSURE_MUTANTS = {
    ...
    "job_changed_bad_grammar": lambda c: _bn(c, "malformed", "job_changed")["request"].update(
        job_id="x"
    ),
    "worker_changed_bad_grammar": lambda c: _bn(c, "malformed", "worker_changed")["request"].update(
        worker="!!"
    ),
