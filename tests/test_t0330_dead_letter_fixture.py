...
single-locus repair makes it bury, except that a corrupt non-dead job
(ready or leased, flagged by minimal_repair.expect_failure) repairs to a
still-not-dead job that fails job_not_dead, which pins job integrity
ahead of the dead check. Rollback cases prove a rejected
...
REPAIR_ND_KEYS = {"request", "expect_failure"}
_ND_CLASS = "job_not_dead"
    ...
    for section in SECTIONS:
        ...
        for row in rows:
            ...
            if section in ("happy", "boundary"):
                ...
                assert type(repair) is dict and set(repair) in ({"request"}, REPAIR_ND_KEYS), label
                if set(repair) == REPAIR_ND_KEYS:
                    # a corrupt non-dead job: its repair is still not dead
                    assert repair["expect_failure"] == _ND_CLASS, label
                    assert row["expect_failure"] == "corrupt_job", label
                    assert repair["request"]["job"]["status"] != "dead", label
...
MALFORMED_MANIFEST = {
    ...
    "payload_key_lone_surrogate": (_CJ, "a payload key is a lone surrogate"),
    "ready_attempts_past_max": (_CJ, "a ready job carries attempts 6, past max_attempts 5"),
    "ready_attempts_below_floor": (_CJ, "a ready job carries attempts -1"),
    "leased_owner_bad_grammar": (
        _CJ,
        "a leased job's lease_owner is '!!', outside the worker grammar",
    ),
    "leased_expiry_past_bound": (_CJ, "a leased job's lease_expires_at is 2**53"),
    "leased_expiry_zero": (_CJ, "a leased job's lease_expires_at is 0"),
...
ROW_DIGESTS = {
    ...
    "malformed:payload_key_lone_surrogate": "4adb14ab3896e8645149ef60f4e34ce3ff2b3606f456572a635e5af3cbb4c45c",  # noqa: E501
    "malformed:ready_attempts_past_max": "965893bd32d03bcdc3f9a740e2a63f0271bd9bf5fe7efa0bd03337a613b47dae",  # noqa: E501
    "malformed:ready_attempts_below_floor": "bce1902187fe67793fe340faf2933a30778b45275691cc319842389ce78870d8",  # noqa: E501
    "malformed:leased_owner_bad_grammar": "98001c0743402807c1e9f96a3f8276172cb89175617e4472cec007015e0f0101",  # noqa: E501
    "malformed:leased_expiry_past_bound": "1d6909e11bcfe0f1ddaa7ac2fb3c45cad1db3c1d1e832588453d61808793b987",  # noqa: E501
    "malformed:leased_expiry_zero": "a82250e9965fcdcff9d9b5d207a14b360451d3bd32dc1872452e00f461be47c7",  # noqa: E501
...
# corrupt non-dead jobs: name -> (status, field, bad value, repaired value);
# each repair is still not dead, so it fails job_not_dead
_NON_DEAD_EDGES = {
    "ready_attempts_past_max": ("ready", "attempts", MAX_ATTEMPTS + 1, MAX_ATTEMPTS),
    "ready_attempts_below_floor": ("ready", "attempts", -1, 0),
    "leased_owner_bad_grammar": ("leased", "lease_owner", "!!", "w-1"),
    "leased_expiry_past_bound": ("leased", "lease_expires_at", MAX + 1, MAX),
    "leased_expiry_zero": ("leased", "lease_expires_at", 0, 1),
}
...
def _m(name, bad, fix):  # noqa: C901 - one closed branch per tag
    ...
    elif name in _NON_DEAD_EDGES:
        status, field, b, f = _NON_DEAD_EDGES[name]
        assert bad["job"]["status"] == fix["job"]["status"] == status, name
        assert _same_type_eq(bad["job"][field], b) and _same_type_eq(fix["job"][field], f), name
        _only(bad, fix, ("job", field))
        if status == "leased":
            assert _same_type_eq(fix["job"]["lease_owner"], "w-1"), name
            assert _int_in_range(fix["job"]["lease_expires_at"], 1, MAX), name
        assert bad["reason"] == _E, name
    ...
    elif name == "payload_key_lone_surrogate":
        assert _same_type_eq(bad["job"]["payload"], {"\ud800": 1})
        assert _same_type_eq(fix["job"]["payload"], {"k": 1})
        _only(bad, fix, ("job", "payload", "\ud800"), ("job", "payload", "k"))
    else:
    ...
    for section, manifest in MANIFESTS.items():
        ...
        for row in rows:
            ...
            if section in ("happy", "boundary"):
                ...
                repair = row["minimal_repair"]
                want = {"expect_failure": _ND_CLASS} if row["name"] in _NON_DEAD_EDGES else {}
                assert {k: v for k, v in repair.items() if k != "request"} == want, label
                _m(row["name"], row["request"], repair["request"])
            else:
...
_CLOSURE_MUTANTS = {
    ...
    "ready_attempts_second_defect": _bad_job("ready_attempts_past_max", priority=5),
    "ready_floor_retyped": _bad_job("ready_attempts_below_floor", attempts=-1.0),
    "leased_owner_second_defect": _bad_job("leased_owner_bad_grammar", seq=4),
    "leased_expiry_bound_shifted": _bad_job("leased_expiry_past_bound", lease_expires_at=MAX + 2),
    "leased_expiry_zero_retyped": _bad_job("leased_expiry_zero", lease_expires_at=False),
    "non_dead_repair_buries": lambda c: _bn(c, "malformed", "leased_expiry_zero")[
        "minimal_repair"
    ].pop("expect_failure"),
    "payload_key_retyped": _bad_job("payload_key_lone_surrogate", payload={"\ud801": 1}),
    "payload_key_value_changed": _bad_job("payload_key_lone_surrogate", payload={"\ud800": 2}),
...
def test_malformed_minimal_repair_succeeds(name):
    """The single-locus repair clears the declared defect: it buries, or
    for a corrupt non-dead job it reaches the dead check and fails
    job_not_dead."""
    row = _row("malformed", name)
    repair = row["minimal_repair"]
    request = copy.deepcopy(repair["request"])
    if "expect_failure" in repair:
        _rejects(repair["expect_failure"], request)
    else:
        _validate_record(_bury(request), name)
    assert _snap(request) == _snap(repair["request"])
...
def _accepted_requests():
    ...
    for row in CASES["malformed"]:
        if "expect_failure" not in row["minimal_repair"]:
