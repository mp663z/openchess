...
def test_clock_overflow_is_measured_from_now_not_base():
    """A partial-carry deny: base (updated_at) stays behind now, so
    now + retry and base + retry differ. Refused at now + retry ==
    2**53 (literal 9007199254740992) even though base + retry is far
    inside the clock; its accepted twin lands exactly on MAX."""
    prev = _forge(cost=3, tokens=0, updated_at=MAX - 999)
    _raises("clock_overflow", req(cost=1, now=MAX - 500, previous=prev))
    assert (MAX - 500) + 501 == 9007199254740992
    assert (MAX - 999) + 501 <= MAX
    twin = _forge(cost=3, tokens=0, updated_at=MAX - 1000)
    got = admit(req(cost=1, now=MAX - 500, previous=twin))
    assert got["decision"] == "deny"
    assert got["retry_after_ms"] == 500
    assert got["updated_at"] == MAX - 1000
    assert (MAX - 500) + got["retry_after_ms"] == MAX
...
CORRUPT = {
    ...
    "capacity-past-bound": _forge(capacity=1_000_001),
    "refill-past-bound": _forge(refill_ms=86_400_001),
    "cost-zero": _forge(cost=0),
    "decision-str-subclass": _forge(decision=_StrSub("admit")),
    ...
    "deny-cost-over-capacity-tokens-0": _forge(
        decision="deny", cost=4, tokens=0, retry_after_ms=4000
    ),
    "decision-unknown-deny-shaped": _forge(decision="hold", cost=2, tokens=0, retry_after_ms=2000),
...
def test_forged_previous_is_otherwise_valid():
    ...
    # accepted twins at each previous-record bound: capacity 1000000 and
    # refill_ms 86400000 (with a matching policy), cost 1
    top_cap = _forge(capacity=1_000_000)
    policy_cap = {"capacity": 1_000_000, "refill_ms": 1000}
    assert (
        admit(req(now=60, policy=policy_cap, previous=top_cap))["previous_id"]
        == top_cap["limit_id"]
    )
    top_refill = _forge(refill_ms=86_400_000)
    policy_refill = {"capacity": 3, "refill_ms": 86_400_000}
    got = admit(req(now=60, policy=policy_refill, previous=top_refill))
    assert got["previous_id"] == top_refill["limit_id"]
...
REFERENCE_EDITS = {
    ...
    "overflow-from-base": (
        "if now + retry > _REF_MAX_NOW:",
        "if base + retry > _REF_MAX_NOW:",
    ),
    "overflow-edge-plus-one": (
        "if now + retry > _REF_MAX_NOW:",
        "if now + retry > _REF_MAX_NOW + 1:",
    ),
    "prev-cost-to-max-capacity": (
        'and _int_in(prev["cost"], 1, prev["capacity"])',
        'and _int_in(prev["cost"], 1, _REF_MAX_CAPACITY)',
    ),
    "prev-decision-enum-off": (
        '        and prev["decision"] in _DECISIONS\n',
        "",
    ),
    "overflow-clamp": (
    ...
    "prev-capacity-upper-plus": (
        'and _int_in(prev["capacity"], 1, _REF_MAX_CAPACITY)',
        'and _int_in(prev["capacity"], 1, _REF_MAX_CAPACITY + 1)',
    ),
    "prev-refill-upper-plus": (
        'and _int_in(prev["refill_ms"], 1, _REF_MAX_REFILL)',
        'and _int_in(prev["refill_ms"], 1, _REF_MAX_REFILL + 1)',
    ),
    "prev-cost-floor-zero": (
        'and _int_in(prev["cost"], 1, prev["capacity"])',
        'and _int_in(prev["cost"], 0, prev["capacity"])',
    ),
    "prev-decision-type-off": ('        and type(prev["decision"]) is str\n', ""),
