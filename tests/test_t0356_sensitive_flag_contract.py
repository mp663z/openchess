...
class FlagStore:
    ...
    def rollback(self, req):
        ...
        if target is False and not approved:
            _fail("approval_missing")
        self._state[cid] = (target, rev + 1, hist[:-1])
        return self.record(cid)
...
MUTANTS = {
    ...
    "rollback_toggle": (
        "self._state[cid] = (target, rev + 1, hist[:-1])",
        "self._state[cid] = (target, rev + 1, hist[:-1] + [sensitive])",
    ),
...
def _battery_green(tmp_path, source):
    copy_ = tmp_path / "test_t0356_battery_copy.py"
...
def test_identity_mutant_is_green(tmp_path):
    assert _battery_green(tmp_path, _edited("_MAX_REV = 2**63 - 1", "_MAX_REV = 2**63 - 1"))


@pytest.mark.parametrize("name", sorted(MUTANTS))
def test_source_mutant_is_killed(name, tmp_path):
    assert not _battery_green(tmp_path, _edited(*MUTANTS[name]))


@pytest.mark.parametrize("name", sorted(EQUIVALENT))
def test_equivalent_edit_stays_green(name, tmp_path):
    assert _battery_green(tmp_path, _edited(*EQUIVALENT[name]))
