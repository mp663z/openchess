...
@pytest.mark.parametrize("op", ["transition", "rollback"])
def test_change_just_below_max_revision_commits_to_max(op):
    s = _store()
    rev = _MAX_REV - 1
    s._state[A] = (True, rev, [False])
    req = dict(_base(op), expected_revision=rev, approval=approval_for(A, rev))
    rec = getattr(s, op)(req)
    if op == "transition":
        assert rec == {"collection_id": A, "sensitive": False, "revision": _MAX_REV, "prior": True}
        assert s._state[A] == (False, _MAX_REV, [False, True])
    else:
        assert rec == {"collection_id": A, "sensitive": False, "revision": _MAX_REV, "prior": None}
        assert s._state[A] == (False, _MAX_REV, [])


def _at_max(s, hist):
    s._state[A] = (True, _MAX_REV, list(hist))
    return s.record(A)


def test_unapproved_lowering_at_max_revision_is_approval_missing():
    s = _store()
    before = _at_max(s, [False])
    req = {"collection_id": A, "sensitive": False, "expected_revision": _MAX_REV, "approval": None}
    _raises("approval_missing", s.transition, req)
    assert s.record(A) == before and s._state[A] == (True, _MAX_REV, [False])


def test_unapproved_rollback_to_false_at_max_revision_is_approval_missing():
    s = _store()
    before = _at_max(s, [False])
    _raises("approval_missing", _rb, s, _MAX_REV)
    assert s.record(A) == before and s._state[A] == (True, _MAX_REV, [False])


def test_rollback_with_empty_history_at_max_revision_is_empty_history():
    s = _store()
    before = _at_max(s, [])
    _raises("empty_history", _rb, s, _MAX_REV, approval_for(A, _MAX_REV))
    assert s.record(A) == before and s._state[A] == (True, _MAX_REV, [])


def test_same_value_set_at_max_revision_is_a_noop():
    # coordinator ruling 22:44: no state and no revision change, so nothing
    # to refuse - same as the no-op at every other revision
    s = _store()
    s._state[A] = (True, _MAX_REV, [False])
    before = s.record(A)
    req = {"collection_id": A, "sensitive": True, "expected_revision": _MAX_REV, "approval": None}
    assert s.transition(req) == before
    assert s._state[A] == (True, _MAX_REV, [False])
...
MUTANTS = {
    ...
    "exh_before_approval_apply": (
        ...
        "        if target is False and not approved:\n"
        '            _fail("approval_missing")\n'
        "        self._state[cid] = (target, new_rev, hist + [sensitive])",
    ),
    "exh_before_approval_rb": (
        "        if target is False and not approved:\n"
        '            _fail("approval_missing")\n'
        "        self._state[cid] = (target, _bump(rev), hist[:-1])",
        "        new_rev = _bump(rev)\n"
        "        if target is False and not approved:\n"
        '            _fail("approval_missing")\n'
        "        self._state[cid] = (target, new_rev, hist[:-1])",
    ),
    "exh_before_empty_rb": (
        '        if not hist:\n            _fail("empty_history")\n',
        '        _bump(rev)\n        if not hist:\n            _fail("empty_history")\n',
