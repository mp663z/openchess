...
LINT_MUTATIONS = {
    ...
    "preimage_target": lambda c: c["identifiers"]["opt_in"]["preimage"].__setitem__("target", "on"),
    ...
    "digest_uppercase": lambda c: c["identifiers"]["opt_in"].__setitem__(
        "digest", "sha256-of-the-preimage-bytes-as-64-uppercase-hex-after-the-lmo1-prefix"
    ),
...
# sha256 of the literal bytes "ins1:" + "1" * 64 + "|0|large-on", computed
# outside the reference; split so no long hex literal sits on one line
_KAT_HEX = "c725805820dbcc51956534506b2b19f9" + "60f67864cb8863606430c9664274c76e"
...
def test_opt_in_preimage_known_answer():
    raw = ("ins1:" + "1" * 64 + "|0|large-on").encode("ascii")
    assert _spec_bytes(INS, 0) == raw
    assert hashlib.sha256(raw).hexdigest() == _KAT_HEX
    assert opt_in_for(INS, 0) == "lmo1:" + _KAT_HEX
    assert _PRE["example"] == "ins1:<64-lowercase-hex>|0|large-on"


@pytest.mark.parametrize("rev", [0, 1, 9, 10, 100, _MAX_REV - 1])
def test_opt_in_matches_the_contract_preimage(rev):
    want = "lmo1:" + hashlib.sha256(_spec_bytes(INS, rev)).hexdigest()
    assert opt_in_for(INS, rev) == want
    m = _world()
    _at(m, rev)
    assert _set(m, "on", rev, INS, want)["large_model"] == "on"


def _variant(tag, iid, rev):
    return {
        "colon_sep": f"{iid}:{rev}:large-on",
        "no_sep": f"{iid}{rev}large-on",
        "trailing_sep": f"{iid}|{rev}|large-on|",
        "leading_sep": f"|{iid}|{rev}|large-on",
        "order_swapped": f"{rev}|{iid}|large-on",
        "target_first": f"large-on|{iid}|{rev}",
        "rev_padded": f"{iid}|{rev:03d}|large-on",
        "rev_signed": f"{iid}|+{rev}|large-on",
        "rev_hex": f"{iid}|{rev:#x}|large-on",
        "space_sep": f"{iid} | {rev} | large-on",
    }[tag]
...
def test_opt_in_from_another_preimage_is_refused(tag, rev):
    m = _world()
    _at(m, rev)
    wrong = "lmo1:" + hashlib.sha256(_variant(tag, INS, rev).encode()).hexdigest()
...
def test_opt_in_utf16_preimage_is_refused():
    m = _world()
    wrong = "lmo1:" + hashlib.sha256(f"{INS}|0|large-on".encode("utf-16")).hexdigest()
