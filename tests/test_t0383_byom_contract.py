...
class ByomStore:
    ...
    def transition(self, req):
        ...
        self._state[(iid, pid)] = (target, _bump(rev))
        return self.record(iid, pid)
...
@pytest.mark.parametrize("to_terms", [True, False], ids=["accept", "revoke"])
def test_change_at_max_revision_fails_typed_state_unchanged(to_terms):
    s, _, _ = _world()
    _at(s, _MAX_REV, None if to_terms else terms_digest(T))
    before = s.record(INS, P)
    _raises("revision_exhausted", _set, s, T if to_terms else None, _MAX_REV)
    assert s.record(INS, P) == before
...
def test_rejected_transition_leaves_everything_unchanged(tag):
    ...
    args = {
        ...
        "exhausted": (None, _MAX_REV, INS, P, None),
