...
- every string example and string array item is UTF-8 encodable (no
  lone surrogate), so canonical_bytes never raises;
...
def _error_schema(shape, closed):
    ...
    if "examples" in code:
        _need(code["examples"][0] in closed, "error code example outside closed_enum")
    return {
        "type": "object",
...
GOLDEN_SHA256 = "01e85ad0f21138263fd953d2790f7886395923de635c48d143d1732f40f89327"
GOLDEN_LENGTH = 17011
...
def _mutants():
    ...
    def privacy_token_dropped(cc):
        cc["privacy"]["rule"] = cc["privacy"]["rule"].replace("-board-eval-", "-board-")

    def idempotency_pattern_dropped(cc):
        cc["idempotency"]["schema"] = "{type: string, minLength: 1}"

    def idempotency_uncited(cc):
        cc["idempotency"]["nonempty_reading"] = "nonempty means minLength 1"
    ...
    return {
        ...
        for f in (
            ...
            depth_bound,
            example_bound,
            privacy_token_dropped,
            idempotency_pattern_dropped,
            idempotency_uncited,
        )
    }
...
def _lint_siblings(monkeypatch, tmp_path, *, mock_text=None, cp_edit=None, repin=True, recite=True):
    """Lint against edited copies of the linked siblings (the real files
    are never touched). By default the pinned mock sha is recomputed for
    an edited mock so the reading check itself is what must fail;
    repin=False keeps the committed pin; recite=False repins the mock but
    leaves the idempotency nonempty_reading citing the committed sha."""
    ...
    sha = hashlib.sha256(mock.read_bytes()).hexdigest() if repin else _STAT["mock_source"]["sha256"]
    doc = yaml.safe_load(CONTRACT.read_text())
    doc["contract"]["error_responses"]["status"]["mock_source"]["sha256"] = sha
    old_sha = _STAT["mock_source"]["sha256"]
    idem = doc["contract"]["idempotency"]
    if recite:
        idem["nonempty_reading"] = idem["nonempty_reading"].replace(old_sha, sha)
    path = tmp_path / "openapi.yaml"
    path.write_text(yaml.safe_dump(doc, sort_keys=False))
    ...
    monkeypatch.setattr(mod, "IDEMPOTENCY", idem)
    mod.lint(path)
...
def _mock_edit(label):
    ...
    if label == "one-site-status-drift":
        ...
        return text[:j] + "status=405" + text[j + len("status=404") :]
    ...
    if label == "bearer-status-changed":
        i = text.index('f"bearer token {reason}"')
        j = text.index("status=401", i)
        return text[:j] + "status=403" + text[j + len("status=401") :]
    if label == "status-not-literal":
        i = text.index('"not_found", "unknown reservation"')
        j = text.index("status=404", i)
        return text[:j] + "status=400 + 4" + text[j + len("status=404") :]
...
MOCK_EDITS = {
    "bearer-status-changed": "computed-code bearer answer",
    "code-outside-enum": "outside the closed enum",
    "contract-status-contradicted": "contradicts the contract",
    "every-site-status-drift": "mock reading drifted",
    "one-site-status-drift": "with statuses",
    "status-not-literal": "not an int literal",
}


@pytest.mark.parametrize("label", sorted(MOCK_EDITS))
def test_lint_binds_the_mock_reading(label, monkeypatch, tmp_path):
    ...
    with pytest.raises(ContractError, match=MOCK_EDITS[label]):
        _lint_siblings(monkeypatch, tmp_path, mock_text=edited)


def test_lint_binds_the_idempotency_citation_to_the_mock_pin(monkeypatch, tmp_path):
    # the mock is edited and repinned; the nonempty reading still cites
    # the old sha
    edited = (ROOT / _CC["links"]["control_plane_mock"]).read_text() + "# edited\n"
    with pytest.raises(ContractError, match="must cite the pinned mock"):
        _lint_siblings(monkeypatch, tmp_path, mock_text=edited, recite=False)


def test_lint_binds_the_mock_bytes(monkeypatch, tmp_path):
    # a harmless edit (a trailing comment) under the committed pin
    edited = (ROOT / _CC["links"]["control_plane_mock"]).read_text() + "# edited\n"
    with pytest.raises(ContractError, match="mock sha256 drifted"):
        _lint_siblings(monkeypatch, tmp_path, mock_text=edited, repin=False)
...
def _cp_error_shape_extra_key(cp):
    cp["contract"]["transport"]["errors"]["shape"]["extra"] = {"fields": {}}


def _cp_error_fields_reordered(cp):
    fields = cp["contract"]["transport"]["errors"]["shape"]["error"]["fields"]
    reordered = {k: fields[k] for k in ("message", "code", "retryable")}
    fields.clear()
    fields.update(reordered)


def _cp_error_code_optional(cp):
    cp["contract"]["transport"]["errors"]["shape"]["error"]["fields"]["code"]["required"] = False


def _cp_error_code_example_outside_enum(cp):
    cp["contract"]["transport"]["errors"]["shape"]["error"]["fields"]["code"]["example"] = (
        "bogus_code"
    )


def _cp_schema_version(cp):
    cp["schema_version"] = 3


def _cp_write_only_truthy_response(cp):
    fields = cp["areas"]["provider_routing"]["ops"]["register_key"]["response"]["fields"]
    fields["key_ref"]["write_only"] = 1


def _cp_surrogate_example(cp):
    fields = cp["areas"]["quota"]["ops"]["reserve"]["request"]["fields"]
    fields["tag"] = {"type": "string", "required": False, "example": "\ud800"}


def _cp_surrogate_item(cp):
    fields = cp["areas"]["quota"]["ops"]["reserve"]["request"]["fields"]
    fields["tags"] = {"type": "array", "items": "string", "required": False, "example": ["\udfff"]}


def _cp_grounding_dropped(cp):
    cp["contract"]["versioning"]["rule"] = cp["contract"]["versioning"]["rule"].replace(
        "MUST ignore unknown request fields", "SHOULD reject unknown request fields"
    )
...
CP_EDITS = {
    f.__name__: (f, m)
    for f, m in (
        (_cp_chess_field, "chess content"),
        (_cp_nested_chess, "chess content"),
        (_cp_write_only_response, "write_only outside a request"),
        (_cp_write_only_truthy_response, "write_only outside a request"),
        (_cp_error_shape, "error fields drifted"),
        (_cp_error_shape_extra_key, "error shape drifted"),
        (_cp_error_fields_reordered, "error fields drifted"),
        (_cp_error_code_optional, "error fields drifted"),
        (_cp_error_code_example_outside_enum, "code example"),
        (_cp_enum_grows, "partition"),
        (_cp_schema_version, "schema_version"),
        (_cp_surrogate_example, "UTF-8"),
        (_cp_surrogate_item, "UTF-8"),
        (_cp_grounding_dropped, "grounding"),
    )
}


@pytest.mark.parametrize("name", sorted(CP_EDITS))
def test_lint_binds_the_control_plane(name, monkeypatch, tmp_path):
    edit, message = CP_EDITS[name]
    with pytest.raises(ContractError, match=message):
        _lint_siblings(monkeypatch, tmp_path, cp_edit=edit)


@pytest.mark.parametrize("token", sorted(CHESS_TOKENS))
def test_lint_refuses_every_chess_token(token, monkeypatch, tmp_path):
    def edit(cp):
        fields = cp["areas"]["quota"]["ops"]["reserve"]["response"]["fields"]
        fields[f"last_{token}_ref"] = {"type": "string", "required": False}

    with pytest.raises(ContractError, match="chess content"):
        _lint_siblings(monkeypatch, tmp_path, cp_edit=edit)


def test_chess_tokens_are_the_privacy_rule_list():
    rule = _CC["privacy"]["rule"]
    listed = rule.split("-token-", 1)[1].split("-split-on-underscore", 1)[0].split("-")
    assert sorted(listed) == sorted(CHESS_TOKENS)
    assert len(CHESS_TOKENS) == 14
...
def test_idempotency_key_schema_is_the_mock_reading_of_nonempty():
    """The header schema accepts exactly what the merged mock accepts:
    not a whitespace-only key (the mock answers 400 for key.strip() == "")."""
    schema = derive(src())["components"]["parameters"]["IdempotencyKey"]["schema"]
    pattern = re.compile(schema["pattern"])

    def schema_accepts(key):
        return len(key) >= schema["minLength"] and pattern.search(key) is not None

    for key in ("", " ", "\t\n ", "\u3000"):
        assert not schema_accepts(key) and not key.strip(), repr(key)
    for key in ("k", " k ", "idem-1"):
        assert schema_accepts(key) and key.strip(), repr(key)
...
def _reserve_field(**spec):
    return _edit(lambda s: op_of(s, "quota.reserve")["request"]["fields"].update(x=spec))


def _error_fields(s):
    return s["contract"]["transport"]["errors"]["shape"]["error"]["fields"]


def _reorder_error_fields(s):
    fields = _error_fields(s)
    reordered = {k: fields[k] for k in ("message", "code", "retryable")}
    fields.clear()
    fields.update(reordered)
...
MALFORMED.update(
    {
        "field-missing-required": _reserve_field(type="string"),
        "boolean-example-1": _reserve_field(type="boolean", required=False, example=1),
        "boolean-example-0": _reserve_field(type="boolean", required=False, example=0),
        "boolean-example-float": _reserve_field(type="boolean", required=False, example=1.0),
        "array-example-mixed": _reserve_field(
            type="array", required=False, items="integer", example=[1, "x"]
        ),
        "string-example-lone-surrogate": _reserve_field(
            type="string", required=False, example="ok\ud800"
        ),
        "string-item-lone-surrogate": _reserve_field(
            type="array", required=False, items="string", example=["ok", "\udfff"]
        ),
        "error-code-example-surrogate": _edit(
            lambda s: _error_fields(s)["code"].update(example="not_found\ud800")
        ),
        "error-code-example-outside-enum": _edit(
            lambda s: _error_fields(s)["code"].update(example="bogus_code")
        ),
        "error-shape-extra-key": _edit(
            lambda s: s["contract"]["transport"]["errors"]["shape"].update(extra={"fields": {}})
        ),
        "error-fields-reordered": _edit(_reorder_error_fields),
        "public-extra-op": _edit(
            lambda s: s["contract"]["transport"]["auth"]["public_operations"].append(
                "quota.reserve"
            )
        ),
        "public-unknown-op": _edit(
            lambda s: s["contract"]["transport"]["auth"]["public_operations"].append("x.y")
        ),
        "read-only-extra-op": _edit(
            lambda s: s["contract"]["transport"]["read_only_operations"].append("quota.reserve")
        ),
        "read-only-unknown-op": _edit(
            lambda s: s["contract"]["transport"]["read_only_operations"].append("x.y")
        ),
    }
)
...
def test_every_chess_token_is_refused():
    assert len(CHESS_TOKENS) == 14
    for token in sorted(CHESS_TOKENS):
        assert _refused(with_field(f"last_{token}_ref")), token
        assert _refused(with_field(token)), token


def test_utf8_string_examples_are_accepted():
    doc = _accepted(with_field(example="é😀\u00a0"))
    schema = _request_schema(doc["paths"]["/quota/reserve"]["post"])
    assert schema["properties"]["extra"]["examples"] == ["é😀\u00a0"]
    _accepted(with_field(type="array", items="string", example=["\ud7ff", "\ue000"]))
    source = src()
    _error_fields(source)["code"]["example"] = "internal"
    error = _accepted(source)["components"]["schemas"]["Error"]
    assert error["properties"]["error"]["properties"]["code"]["examples"] == ["internal"]
...
HOSTILE = {
    ...
    "area-note-str-subclass": _edit(lambda s: s["areas"]["billing"].update(pci_boundary=S("x"))),
...
REFERENCE_EDITS = {
    "idempotency-pattern-dropped": (
        '"schema": {"type": "string", "minLength": 1, "pattern": "\\\\S"},',
        '"schema": {"type": "string", "minLength": 1},',
    ),
    "bool-example-by-value": ("return type(value) is bool", "return value in (True, False)"),
    "array-example-any": ("all(_example_ok(item, None, v)", "any(_example_ok(item, None, v)"),
    "area-extra-isinstance": ("type(aspec[key]) is str", "isinstance(aspec[key], str)"),
    "public-subset": (
        "_need(sorted(publics) == sorted(public),",
        "_need(set(publics) <= set(public),",
    ),
    "read-only-subset": (
        "_need(sorted(read_onlys) == sorted(read_only),",
        "_need(set(read_onlys) <= set(read_only),",
    ),
    "surrogates-allowed": (
        'return type(value) is str and not any("\\ud800" <= ch <= "\\udfff" for ch in value)',
        "return type(value) is str",
    ),
    "code-example-unchecked": (
        '_need(code["examples"][0] in closed, "error code example outside closed_enum")',
        "pass",
    ),
...
EQUIVALENT_EDITS = {
    # the required list follows field order and is pinned to
    # [code, message, retryable], so a reordered shape is refused there
    "error-fields-as-set": (
        '_need(list(dict.keys(fields)) == ["code", "message", "retryable"], "error fields")',
        '_need(set(dict.keys(fields)) == {"code", "message", "retryable"}, "error fields")',
    ),
