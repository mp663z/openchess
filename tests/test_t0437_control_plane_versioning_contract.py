...
# EQUIVALENT, not killed: T0419 strict source validation rejects the
# malformed snapshot before any weakened local compatibility guard runs.
def _eq_duplicate(new):
    new["contract"]["transport"]["read_only_operations"].append("entitlements.get")


def _eq_missing(new):
    new["contract"]["transport"]["read_only_operations"].remove("entitlements.get")


def _eq_extra(new):
    new["contract"]["transport"]["auth"]["public_operations"].append("quota.reserve")
...
def _eq_duplicate_route(new):
    new["areas"]["identity"]["ops"]["new_login"] = copy.deepcopy(new["areas"]["identity"]["ops"]["login"])


def _eq_old_route(new):
    op = copy.deepcopy(new["areas"]["identity"]["ops"]["login"])
    op["path"] = new["areas"]["identity"]["ops"]["refresh"]["path"]
    new["areas"]["identity"]["ops"]["new_login"] = op


# Every v1 named survivor, plus the v2 allowlist-block cases.
EQUIVALENT = {
    "duplicate-allowlist-check-deleted": (
        "len(new_list) != len(set(new_list)) or ", "", _eq_duplicate),
    "set-equality-deleted": (
        "set(new_list) != set(old_list) | set(expected)", "False", _eq_missing),
    "set-equality-weakened-to-subset": (
        "set(new_list) != set(old_list) | set(expected)",
        "not set(new_list).issubset(set(old_list) | set(expected))", _eq_missing),
    "set-equality-weakened-to-superset": (
        "set(new_list) != set(old_list) | set(expected)",
        "not set(new_list).issuperset(set(old_list) | set(expected))", _eq_extra),
    "len-added-check-deleted": (
        "or len(added) != len(expected)", "", _eq_missing),
    "schema-version-check-deleted": (
        'if old["schema_version"] != new["schema_version"]:\n        return False',
        "if False:\n        return False", _eq_schema),
    "duplicate-route-check-deleted": (
        "if route in new_routes:\n                return False",
        "if False:\n                return False", _eq_duplicate_route),
    "old-route-reuse-check-deleted": (
        "and route in old_routes:", "and False:", _eq_old_route),
}


@pytest.mark.parametrize("name", EQUIVALENT)
def test_equivalent_guard_is_preempted_by_strict_source(name):
    before, replacement, probe = EQUIVALENT[name]
    source = inspect.getsource(_fields) + "\n" + inspect.getsource(_minor) + "\n" + inspect.getsource(compare)
    assert source.count(before) == 1, name
    scope = dict(globals())
    exec(compile(ast.parse(source.replace(before, replacement, 1)),
                 f"<equivalent:{name}>", "exec"), scope)
    old, new = fresh()
    probe(new)
    for model in (compare, scope["compare"]):
        with pytest.raises(VersionError) as error:
            model(old, new, 0, 1)
        assert error.value.code == "malformed_request"
        assert error.value.__cause__ is None and error.value.__context__ is None
