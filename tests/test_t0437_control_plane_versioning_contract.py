...
REFERENCE_MUTANTS = {
    "new-required-response-accepted": ("spec.get(\"required\") is not False", "side == \"request\" and spec.get(\"required\") is not False", "test_minor_and_major_edge"),
    "optional-to-required-response-accepted": ("elif spec != current:\n            return False", "elif spec != current and side == \"request\":\n            return False", "test_minor_and_major_edge"),
    "new-required-request-accepted": ("spec.get(\"required\") is not False", "side == \"response\" and spec.get(\"required\") is not False", "test_new_required_request_field_refused_minor_accepted_major"),
    "optional-to-required-request-accepted": ("elif spec != current:\n            return False", "elif spec != current and side == \"response\":\n            return False", "test_minor_and_major_edge"),
    "nested-optional-rejected": ("if not _fields(spec[\"fields\"], current[\"fields\"], side):", "if spec[\"fields\"] != current[\"fields\"]:", "test_nested_optional_addition_is_minor"),
    "base-path-not-required-for-major": ("if newer > older:\n        return True", "if newer >= older:\n        return True", "test_no_base_change_cannot_claim_major"),
    "minor-missing-ceiling": ("new_minor <= 2147483647", "new_minor <= 2147483648", "test_bad_minors_are_typed_and_atomic"),
    "minor-downgrade-accepted": ("new_minor < old_minor", "new_minor > old_minor", "test_bad_minors_are_typed_and_atomic"),
    "operation-route-change-minor": ("current.get(k) != value", "False", "test_existing_shape_changes_are_major_only"),
    "strict-validator-removed": ("strict_source_derivation(doc)", "_base(doc)", "test_source_validator_is_t0419_strict_on_new_optional_nested_bad_type"),
}


def _mutant_compare(name):
    old, replacement, _ = REFERENCE_MUTANTS[name]
