...
def _fen_placement_in(text):
    # Linear in the string length: split once, then slide over eight
    # slash-delimited segments. First/last may be a rank suffix/prefix;
    # each rank has at most eight characters, so at most eight bounded
    # candidates per edge. There are no outside-edge exclusions.
    def rank_ok(rank):
        if not 1 <= len(rank) <= FEN_PLACEMENT["rank_sum"]:
            return False
        total = 0
        prior_digit = False
        for ch in rank:
            if ch in FEN_PIECES:
                total += 1
                prior_digit = False
            elif ch in FEN_PLACEMENT["empty_run_digits"] and not prior_digit:
                total += int(ch)
                prior_digit = True
            else:
                return False
        return total == FEN_PLACEMENT["rank_sum"]

    segments = text.split(FEN_PLACEMENT["rank_separator"])
    n = FEN_PLACEMENT["rank_count"]
    if len(segments) < n:
        return False
    full = [rank_ok(segment) for segment in segments]
    edge = FEN_PLACEMENT["rank_sum"]
    prefix = [any(rank_ok(segment[:end]) for end in range(1, min(len(segment), edge) + 1))
              for segment in segments]
    suffix = [any(rank_ok(segment[-length:]) for length in range(1, min(len(segment), edge) + 1))
              for segment in segments]
    bad_middle = [0]
    for valid in full:
        bad_middle.append(bad_middle[-1] + (not valid))
    for start in range(len(segments) - n + 1):
        if (suffix[start] and prefix[start + n - 1]
                and bad_middle[start + n - 1] == bad_middle[start + 1]):
            return True
    return False
...
REFERENCE_MUTANTS = {
    ...
    "nine-ranks-accepted": ('n = FEN_PLACEMENT["rank_count"]', 'n = FEN_PLACEMENT["rank_count"] + 1', "nine_ranks"),
    "rank-count-less-than": ('n = FEN_PLACEMENT["rank_count"]', 'n = FEN_PLACEMENT["rank_count"] - 1', "seven_ranks"),
    "greedy-no-backtrack": ('for start in range(len(segments) - n + 1):',
                            'for start in range(0):', "greedy"),
    "first-window-only": ('range(len(segments) - n + 1)', 'range(1)', "later_window"),
    "middle-rank-off-by-one": ('bad_middle[start + n - 1] == bad_middle[start + 1]',
                               'bad_middle[start + n - 2] == bad_middle[start + 1]', "middle_bad"),
    "quadratic-offset-scan": ('segments = text.split(FEN_PLACEMENT["rank_separator"])',
                              'for start_offset in range(len(text)):\n        for end_offset in range(start_offset, len(text)):\n            if end_offset < 0: return False\n    segments = text.split(FEN_PLACEMENT["rank_separator"])', "timing"),
    "left-edge-digit-skip": ('segment[-length:]', 'segment[-length:] if not segment.startswith("1") else ""', "left_edge"),
    "right-edge-digit-skip": ('segment[:end]', 'segment[:end] if not segment.endswith("9") else ""', "right_edge"),
...
def test_reference_mutant_red(name):
    ...
    elif name == "quadratic-offset-scan":
        large = {"extra": "p" * 4096}
        started = time.perf_counter()
        assert classify(source, 200, large) == {"kind": "success"}
        fast = time.perf_counter() - started
        started = time.perf_counter()
        assert target(source, 200, large) == {"kind": "success"}
...
@pytest.mark.parametrize("placement", ["8/8/8/8/8/8/8", "8p/8p/8p/8p/8p/8p/8p/8p", "7/8/8/8/8/8/8/8", "44/8/8/8/8/8/8/8"])
...
@pytest.mark.parametrize("candidate", [
    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNRb",
    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNRbest",
    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNRk",
    "fen=rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNRq",
    "4k3/8/8/8/8/8/8/4K3p",
    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR9",
    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR0",
    "1rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR",
    "18/8/8/8/8/8/8/8",
])
def test_every_start_end_offset_valid_fen_substring_refused(candidate):
    with pytest.raises(ErrorsError):
        classify(SOURCE, 200, {"extra": candidate})


@pytest.mark.parametrize("candidate", ["08/8/8/8/8/8/8/8", "0p7/8/8/8/8/8/8/8"])
def test_zero_digit_with_valid_suffix_window_refused(candidate):
    with pytest.raises(ErrorsError):
        classify(SOURCE, 200, {"extra": candidate})


def test_linear_fen_scan_at_declared_collection_limit():
    extras = {f"x{i}": "p" * 65536 for i in range(256)}
    extras["slash_dense"] = ("p/" * 32768)[:65536]
    # Root dict bound 256: the slash-dense value takes the final slot.
    extras.pop("x255")
    started = time.perf_counter()
    assert classify(SOURCE, 200, extras) == {"kind": "success"}
    assert time.perf_counter() - started < 8.0


@pytest.mark.parametrize("prefix", ["x/", "1/2 "])
def test_fen_window_after_slash_separated_prefix_refused(prefix):
    placement = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR"
    with pytest.raises(ErrorsError):
        classify(SOURCE, 200, {"extra": prefix + placement})


@pytest.mark.parametrize("bad", ["x", "9"])
@pytest.mark.parametrize("position", range(1, 7))
def test_one_invalid_middle_rank_at_each_position_allowed(bad, position):
    ranks = ["8"] * 8
    ranks[position] = bad
    assert classify(SOURCE, 200, {"extra": "/".join(ranks)}) == {"kind": "success"}


@pytest.mark.parametrize("placement", [
    "8/8/x/8/8/8/8/8", "8/8/8/8/8/8/9/8",
    "rnbqkbnr/pppppppp/8/8/8/8/x/RNBQKBNR",
])
