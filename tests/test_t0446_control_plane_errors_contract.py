...
def _fen_placement_in(text):
    ...
    for part in parts:
        ranks = part.split(FEN_PLACEMENT["rank_separator"])
        if len(ranks) != FEN_PLACEMENT["rank_count"]:
            continue
        if all(rank and all(ch in FEN_PIECES or (ch.isascii() and ch.isdigit() and int(ch) in FEN_DIGITS) for ch in rank)
               and sum(int(ch) if ch.isdigit() else 1 for ch in rank) == FEN_PLACEMENT["rank_sum"]
               for rank in ranks):
            return True
    return False
...
def _json_safe(value, depth=0, allowed=frozenset(), fen_check=True):
    if depth > 8:
        return False
    if value is None or type(value) in (bool, int):
        return True
    if type(value) is str:
        return _text(value) and (not fen_check or not _fen_placement_in(value))
    if type(value) is float:
        return value == value and abs(value) != float("inf")
    if type(value) is list:
        return len(value) <= 256 and all(_json_safe(v, depth + 1, fen_check=fen_check) for v in value)
    if type(value) is dict:
        return len(value) <= 256 and all(_safe_key(k, k in allowed) and _json_safe(v, depth + 1, fen_check=fen_check) for k, v in value.items())
    return False
...
def classify(source, status, payload, operation=None):
    ...
    if operation is not None:
        ...
        try:
            ...
            _fail("operation")
    if type(payload) is not dict or not _json_safe(payload, allowed=declared, fen_check=status >= 400):
        _fail("payload")
    if status < 300:
        return {"kind": "success"}
...
REFERENCE_MUTANTS = {
    ...
    "success-payload-validation-skipped": ("if type(payload) is not dict or not _json_safe(payload, allowed=declared, fen_check=status >= 400):", "if status >= 400 and (type(payload) is not dict or not _json_safe(payload, allowed=declared)):", "hostile_2xx"),
    "low-surrogate-allowed": ('"\\udfff"', '"\\udbff"', "low_surrogate"),
    "extra-string-surrogate-allowed": ("return _text(value) and (not fen_check or not _fen_placement_in(value))", "return not _fen_placement_in(value)", "extra_surrogate"),
    ...
    "nested-declared-exemption": ("_json_safe(v, depth + 1, fen_check=fen_check) for k, v", "_json_safe(v, depth + 1, allowed, fen_check=fen_check) for k, v", "nested_declared"),
    "operation-extra-segment": ('operation.split(".")', 'operation.split(".")[:2]', "operation_extra"),
    "camelcase-boundary-lost": ('re.sub(r"([a-z0-9])([A-Z])", r"\\1_\\2", key)', 'key', "camelcase"),
    "fen-value-check-lost": ("and (not fen_check or not _fen_placement_in(value))", "", "fen"),
}
...
@pytest.mark.parametrize("key", ("gameMoves", "providerKey", "GameMoves"))
