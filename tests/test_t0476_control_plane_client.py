...
SECRET = "sk-live-" + "0123456789abcdef"  # split: a key-shaped literal trips the repo secret scan
...
def _switch_account(M, via):
    ...
    other = {"email": "b@example.com", "password_hash_client": "hb"}
    if via == "login":
        M.ControlPlaneClient(transport).call("identity.register", dict(other))
    c = M.ControlPlaneClient(transport)
    c.call("identity.register", dict(EMAIL))
    c.entitlements()  # A's entitlements cached
    a_account = c.account_id
    c.call("identity.register" if via == "register" else "identity.login", dict(other))
    assert c.account_id != a_account
    state["down"] = True
    assert c.entitlements() == prod.FREE_TIER and c.entitlements_source == "free"


def check_login_as_another_account_never_serves_its_cache(M):
    _switch_account(M, "login")


def check_register_another_account_never_serves_its_cache(M):
    _switch_account(M, "register")


def check_logout_forgets_tracked_reservations(M):
    c, wire, server, clock, _ = _registered(M)
    r = c.call("quota.reserve", dict(RESERVE))
    c.call("identity.logout")
    c.call("identity.login", dict(EMAIL))
    clock.now = r["expires_at"]  # would short-circuit if still tracked
    n = len(wire.calls)
    with contextlib.suppress(M.ControlPlaneError):  # only the send is pinned
        c.call("quota.commit", {"reservation_id": r["reservation_id"], "actual_units": 1})
    assert _paths(wire)[n:] == ["/quota/commit"]
...
HOSTILE_RESULTS = {
    ...
    "status_600": (600, {}),
    "status_99": (99, {}),
    "status_600_valid_error": (600, _err()),
    "status_99_valid_error": (99, _err()),
    "status_600_valid_success": (
        600,
        {"status": "a", "tier": "f", "current_period_end": 1, "cancel_at_period_end": False},
    ),
    "tuple_sub_hostile_len_iter": None,  # filled in below (needs CALLS)
...
HOSTILE_RESULTS["tuple_sub_hostile_len_iter"] = HostileTuple((200, dict(SUB_OK)))
...
def _nested_secret(depth):
    out = SECRET
    for i in range(depth):
        out = {"n": out} if i % 2 else [out]
    return out


def _cyclic_then_secret():
    loop = []
    loop.append(loop)
    loop.append({"after": SECRET})
    return [loop, loop]


ECHO = {
    "undeclared_top_value": SECRET,
    "undeclared_nested_50": _nested_secret(50),
    "undeclared_dict_key": {SECRET: 1},
    "undeclared_substring": f"prefix-{SECRET}-suffix",
    "after_cyclic_list": _cyclic_then_secret(),
}


def _echo_payload(label):
    if label == "write_only_key_name":
        return {"key_ref": "r1", "key_material": "harmless"}
    return {"key_ref": "r1", "extra": ECHO[label]}


def echo_refused(M, label):
    c = _armed_client(M, (200, _echo_payload(label)))
    before = _state(c)
    CALLS.clear()
    err = _expect(
        M,
    ...
    assert SECRET not in str(err) and SECRET not in err.message
    assert _state(c) == before and CALLS == []


ECHO_LABELS = sorted([*ECHO, "write_only_key_name"])
...
@pytest.mark.parametrize("label", ECHO_LABELS)
def test_secret_echo_anywhere_in_a_success_payload_is_internal(label):
    echo_refused(prod, label)
...
MUTANTS = {
    ...
    "contains_key_not_scanned": (
        "                stack.append(k)\n",
        "",
    ),
    "contains_scan_truncated": (
        "            if id(item) in seen:",
        "            if id(item) in seen or len(seen) > 3:",
    ),
    "contains_write_only_name_ignored": (
        "                if type(k) is str and k in _SECRET_NAMES:\n                    return True\n",  # noqa: E501
        "",
    ),
    "contains_recursive": (
...
EQUIVALENT = {
    "project_array_uncopied": (
        "list(value) -> value in _project: call() and the cache deep-copy the "
        "projected payload, whose declared array items are plain str"
    ),
...
def _battery(module):
    ...
    yield from ((echo_refused, label) for label in ECHO_LABELS)
