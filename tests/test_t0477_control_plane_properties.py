...
def _required_leaves(fields, prefix=()):
    for name, spec in fields.items():
        path = (*prefix, name)
        if spec["type"] == "object":
            if spec["required"]:
                yield path, "object"
            yield from _required_leaves(spec["fields"], path)
        elif spec["required"]:
            yield path, spec["type"]


def _hostile_cases():
    cases = []
    for area, spec in DOC["areas"].items():
        for name, op in spec["ops"].items():
            for path, kind in _required_leaves(op["request"]["fields"]):
                invalids = (True, 1.0) if kind in ("integer", "number") else (1,)
                for value in invalids:
                    cases.append((f"{area}.{name}", path, value))
    # The optional nested policy must still reject non-objects.
    cases.extend(("provider_routing.route", ("policy",), v) for v in (1, [], "own-key"))
    return cases


@pytest.mark.parametrize("operation,path,invalid", _hostile_cases(),
                         ids=lambda v: str(v))
def test_each_required_leaf_and_optional_object_reject_hostile_type(operation, path, invalid):
    area, op = operation.split(".")
    ...
    before = len(wire.calls)
    assert _outcome(lambda: c.call(operation, body)) == ("error", "malformed_request")
    assert len(wire.calls) == before
...
def test_mutating_transport_cannot_change_caller_body_or_sent_snapshot():
    observed = []
    original = {"capability": "analysis-fast", "policy": {"prefer": "own-key", "max_cost_usd": 0.05}}
    ...
    def transport(method, path, headers, body):
        ...
        if path != "/provider-routing/route":
            return mock.handle(method, path, headers, body)
        body["policy"]["prefer"] = "mutated"
        body["policy"]["unexpected"] = ["hostile"]
        body["capability"] = "mutated"
        if len([item for item in observed if item.get("capability") == "analysis-fast"]) == 1:
            raise ConnectionError("transport changed its own copy before losing reply")
        return mock.handle(method, path, headers, observed[-1])

    c = prod.ControlPlaneClient(transport, max_attempts=2)
    c.call("identity.register", dict(EMAIL))
    c.call("provider_routing.route", original)
    assert original == snapshot
    assert observed[-2:] == [snapshot, snapshot]


def test_nested_cost_caps_extra_is_projected_away():
    class Extra(MockControlPlane):
        def _op_entitlements_get(self, body, account=None, headers=None):
            status, payload = super()._op_entitlements_get(body, account, headers)
            payload["cost_caps"]["future_cap"] = {"nested": [1, 2]}
            return status, payload

    c = prod.ControlPlaneClient(Extra().handle)
    c.call("identity.register", dict(EMAIL))
    got = c.call("entitlements.get")
    assert "future_cap" not in got["cost_caps"]
    assert set(got["cost_caps"]) == set(prod.OPS["entitlements.get"]["response"]["cost_caps"]["fields"])


def test_closed_enum_error_remains_typed():
    mock = MockControlPlane()

    def transport(method, path, headers, body):
        if path == "/identity/register":
            return 429, {"error": {"code": "rate_limited", "message": "slow down", "retryable": False}}
        return mock.handle(method, path, headers, body)

    c = prod.ControlPlaneClient(transport, max_attempts=1)
    with pytest.raises(prod.ControlPlaneError) as error:
        c.call("identity.register", dict(EMAIL))
    assert error.value.code == "rate_limited"
    assert error.value.status == 429 and error.value.retryable is False
