...
NEW_GAME2_INDEX_TMP = 1 + 5 + 3
NEW_SUMMARY_TMP = 1 + 5 * 3 + 1 + 1
NEW_GAME3_INDEX_TMP = NEW_GAME3_RECORD_TMP + 2
...
def neg_root_tmp_refused_restart(sc, tmp, n, prefix, label, root_tmp, corrupt=1):
    """Crash with a store-ROOT temp torn (index.json.tmp or summary.json.tmp);
    the restart commits nothing (game 1 duplicate, game 2 corrupted and
    refused), so no later commit or summary write can overwrite the stale
    temp: recovery itself must remove it."""
    if sc.kinds[n - 1] != "w":
        return ["write layout changed"]
    store, crashed = _crash(sc, label, n, "torn")
    if not crashed or [p.name for p in store.glob("*.tmp")] != [root_tmp]:
        return [f"residue lacks exactly the torn root temp {root_tmp}"]
    games = _split_games(CORPUS)
    games[corrupt] = _corrupt_game(games[corrupt])
    bad_pgn = _write(tmp / f"{label}.pgn", "\n".join(g.rstrip("\n") + "\n" for g in games))
    before_events = events(store)
    rp = _restart(sc, store, pgn=bad_pgn)
    bad = []
    if rp.returncode != 1 or json.loads(rp.stderr or "{}").get("code") != "malformed_request":
        bad.append("corrupted game not refused as malformed_request")
    index, stored = committed(store)
    if index != sc.ref_index[:prefix]:
        bad.append("committed prefix changed")
    if set(stored) != {"games/" + e["record"] for e in index or []}:
        bad.append("unreferenced record after recovery")
    if any(p.endswith(".tmp") for p in tree(store)):
        bad.append("stale .tmp survived recovery")
    if (
        events(store)[len(before_events) :]
        != ["import.started"] + ["import.game_duplicate"] * corrupt
    ):
        bad.append("restart telemetry")
    if (store / "summary.json").exists():
        bad.append("summary written by a refused run")
...
def test_root_index_tmp_is_swept_by_a_restart_that_commits_nothing(new_import, tmp_path):
    assert (
        neg_root_tmp_refused_restart(
            new_import, tmp_path, NEW_GAME2_INDEX_TMP, 1, "idx", "index.json.tmp"
        )
        == []
    )


def test_root_summary_tmp_is_swept_by_a_restart_that_commits_nothing(new_import, tmp_path):
    assert (
        neg_root_tmp_refused_restart(
            new_import, tmp_path, NEW_SUMMARY_TMP, 3, "sum", "summary.json.tmp"
        )
        == []
    )


def test_root_index_tmp_of_game3_is_swept_by_a_refused_restart(new_import, tmp_path):
    bad = neg_root_tmp_refused_restart(
        new_import, tmp_path, NEW_GAME3_INDEX_TMP, 2, "idx3", "index.json.tmp", corrupt=2
    )
    assert bad == []
...
MUTANTS = {
    ...
    "recovery-root-sweep-dropped": (
        'for tmp in list(store_root.glob("*.tmp")) + list(games_dir.glob("*.tmp")):',
        'for tmp in list(games_dir.glob("*.tmp")):',
    ),
...
def battery_red(new, upd, tmp):
    ...
    checks = (
        ...
        lambda: neg_root_tmp_refused_restart(
            new, tmp, NEW_GAME2_INDEX_TMP, 1, "idx", "index.json.tmp"
        ),
        lambda: neg_root_tmp_refused_restart(
            new, tmp, NEW_SUMMARY_TMP, 3, "sum", "summary.json.tmp"
        ),
        lambda: neg_root_tmp_refused_restart(
            new, tmp, NEW_GAME3_INDEX_TMP, 2, "idx3", "index.json.tmp", corrupt=2
        ),
