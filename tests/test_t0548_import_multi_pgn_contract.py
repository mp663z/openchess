...
def test_empty_or_whitespace_input_refuses_without_state(text):
    store = ReferenceStore()
    with pytest.raises(Refusal) as error:
        PRODUCTION_BINDING(text, store)
    assert error.value.code == "malformed_request"
    assert store.index == [] and store.records == {} and store.summary is None
    assert store.telemetry == ["import.started"]


def test_single_newline_between_games_is_not_a_splitter_boundary_through_binding():
    store = ReferenceStore()
    with pytest.raises(Refusal) as error:
        PRODUCTION_BINDING(VALID_PGN.rstrip() + "\n" + _other(), store)
    assert error.value.code == "malformed_request"
    assert store.index == [] and store.records == {} and store.summary is None
    assert store.telemetry == ["import.started"]


def _edited_header_game():
    from tests.test_t0537_import_pgn_contract import FULL_HEADER_PGN

    return FULL_HEADER_PGN, FULL_HEADER_PGN.replace("2. Nf3 Nc6", "2. Nf3 d6")


def test_update_then_duplicate_by_id_across_imports():
    old, edited = _edited_header_game()
    store = ReferenceStore()
    first = PRODUCTION_BINDING(old, store)
    assert first["games_imported"] == 1
    old_entry = copy.deepcopy(store.index[0])
    old_record = copy.deepcopy(store.records[old_entry["record"]])
    before = len(store.telemetry)
    result = PRODUCTION_BINDING(edited, store)
    assert result == {"kind": "import-summary", "source_id": "pgn-multi",
                      "games_imported": 0, "games_already_imported": 0, "games_updated": 1}
    assert store.telemetry[before:] == ["import.started", "import.game_updated", "import.completed"]
    assert len(store.index) == 1 and store.index[0]["game_id"] == old_entry["game_id"]
    assert store.index[0]["record"] != old_entry["record"]
    assert store.records[old_entry["record"]] == old_record
    assert store.records[store.index[0]["record"]]["content_sha256"] != old_record["content_sha256"]
    # Repeating the edit is a duplicate, not a second update or new game.
    before = len(store.telemetry)
    again = PRODUCTION_BINDING(edited, store)
    counts = (again["games_imported"], again["games_already_imported"], again["games_updated"])
    assert counts == (0, 1, 0)
    assert len(store.index) == 1 and store.telemetry[before:] == [
        "import.started", "import.game_duplicate", "import.completed"]


def test_same_file_has_stored_then_updated_and_one_live_index_entry():
    old, edited = _edited_header_game()
    store = ReferenceStore()
    result = PRODUCTION_BINDING(old + "\n" + edited, store)
    assert result == {"kind": "import-summary", "source_id": "pgn-multi",
                      "games_imported": 1, "games_already_imported": 0, "games_updated": 1}
    assert store.telemetry == ["import.started", "import.game_stored",
                               "import.game_updated", "import.completed"]
    assert len(store.index) == 1 and len(store.records) == 2
    active = store.records[store.index[0]["record"]]
    assert active["movetext"].endswith("Nf3 d6 1-0")
    assert len(set(store.records) - {store.index[0]["record"]}) == 1


def _independent_game_id(tags, movetext):
    headers = ("Site", "White", "Black", "UTCDate", "Date", "Round")
    if all(tags.get(key, "").strip() for key in headers):
        preimage = ["headers", [tags[key] for key in headers]]
    else:
        preimage = ["content", {"tags": dict(sorted(tags.items())), "movetext": movetext}]
    raw = json.dumps(preimage, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    return hashlib.sha256(raw).hexdigest()[:16]


@pytest.mark.parametrize("full_headers", [False, True])
def test_known_answer_game_identity_in_both_tiers_with_non_ascii_tag(full_headers):
    from tests.test_t0537_import_pgn_contract import FULL_HEADER_PGN

    text = FULL_HEADER_PGN if full_headers else VALID_PGN
    text = text.replace('[Event "Test"]', '[Event "Café 世界"]')
    store = ReferenceStore()
    PRODUCTION_BINDING(text, store, retrieval_detail="unicode-source.pgn")
    rec = store.records[store.index[0]["record"]]
    assert rec["game_id"] == _independent_game_id(rec["tags"], rec["movetext"])
    assert rec["game_id"] == store.index[0]["game_id"]
    assert rec["tags"]["Event"] == "Café 世界"
    assert rec["provenance"]["retrieval_detail"] == "unicode-source.pgn"
