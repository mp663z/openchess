...
def test_second_game_truncated_has_game_relative_line():
    store = ReferenceStore()
    result = PRODUCTION_BINDING(VALID_PGN + "\n" + TRUNCATED[0], store, scenario=SCENARIO)
    assert result.marker["location"] == {"game_number": 2,
                                          "line": len(TRUNCATED[0].strip().splitlines())}
    assert result.game_number == 2 and store.telemetry == SCENARIO["telemetry"]
    assert store.index == [] and store.summary is None
...
def test_wrong_source_refused_without_effects():
    ...
    with pytest.raises(Refusal) as error:
        PRODUCTION_BINDING(TRUNCATED[0], store, source_id="lichess-public", scenario=SCENARIO)
