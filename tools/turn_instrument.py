"""T0057: turn instrumentation - a diagnostic trace layer over the
T0053 turn runtime (O2 trusted diagnosis).

TurnTracer wraps an apply function (default turn_runtime.apply_move)
and records ONE structured trace record per call:

- accept: {"seq", "outcome": "accept", "move", "terminated",
  "state_before", "state_after"}
- reject: {"seq", "outcome": "reject", "move", "terminated",
  "state_before", "failure_class", "code", "retryable", "message"} -
  the exact TurnError fields, class-to-code mapping included.
- crash: {"seq", "outcome": "crash", "move", "terminated",
  "state_before", "error_type"} - any non-TurnError BaseException
...
class TurnTracer:
    ...
    def __init__(self, apply_fn=rt.apply_move):
    ...
    def apply(self, state, move, terminated=None):
        base = {
            "seq": len(self._records),
            "move": _safe_snapshot(move),
            "terminated": _safe_snapshot(terminated),
            "state_before": _safe_snapshot(state),
        }
        try:
            out = self._apply(state, move, terminated)
